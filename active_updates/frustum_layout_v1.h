...
#define FGLS_SHADOW_COUNT     27        /* trit-cube 3³ (was 28=4×7, bad prime) */
#define FGLS_POLAR_COUNT       3        /* icosa poles N/S/C            */
...
typedef struct __attribute__((packed)) {
    ...
    /* shadow_bitmap: 34B — 27 shadow (3³) + 3 polar + 4B flags
     * shadow_state[0..26]: trit-cube slots (3³, pure {3} family)
     *   bit0 = occupied
     *   bit1 = heptagon_fence (one-way valve)
     *   bit2-7 = fence_key (6 bits, 12 gate states)
     * polar_state[0..2]: icosa poles N/S/C
     *   bit0 = occupied  bit1 = fence
     * threshold: shadow(27×1) + drain(12×2) + polar(3×1) = 54 ✓
     * offset in meta: 448 */
    uint8_t  shadow_state[FGLS_SHADOW_COUNT];       /* 27B */
    uint8_t  polar_state[FGLS_POLAR_COUNT];         /*  3B */
    uint32_t shadow_ctrl;                           /*  4B  → total 34B */
    ...
    /* reserved: 574B (was 576B, -2B absorbed by polar_state[3])
    ...
    /* meta total: 144+288+16+34+384+574 = 1440B ✓ */
} FrustumMeta;
 ...
 *   offset 3904 : meta.shadow_state  27B  ← was 28
 *   offset 3931 : meta.polar_state   3B   ← NEW
 *   offset 3934 : meta.shadow_ctrl   4B
 *   offset 3938 : meta.merkle_roots  384B
 *   offset 4322 : meta.reserved      574B ← header lives here [0..16]
 *   offset 4896 : END
 */
typedef struct __attribute__((packed)) {
