...
/* ═══════════════════════════════════════════════════════════════════
   A11 — real path threshold (no mock): weighted 27×1 + 12×2 + 3×1 = 54
   Verifies patch: SHADOW_COUNT=27 + POLAR_COUNT=3, drain weight=2
   ═══════════════════════════════════════════════════════════════════ */
static void test_A11(void)
{
    printf("[A11] real-path threshold: shadow×1 + drain×2 + polar×1 = 54\n");

    FrustumBlock b; block_init(&b);

    /* fill all 27 shadow slots occupied */
    for (int i = 0; i < FGLS_SHADOW_COUNT; i++)
        b.meta.shadow_state[i] = SHADOW_BIT_OCCUPIED | SHADOW_BIT_FENCE;

    /* fill all 12 drain slots active */
    for (int i = 0; i < FGLS_DRAIN_COUNT; i++)
        b.meta.drain_state[i] = DRAIN_BIT_ACTIVE | DRAIN_BIT_FLUSH;

    /* fill all 3 polar slots occupied */
    for (int i = 0; i < FGLS_POLAR_COUNT; i++)
        b.meta.polar_state[i] = SHADOW_BIT_OCCUPIED;

    /* weighted sum: 27×1 + 12×2 + 3×1 = 27+24+3 = 54 */
    int shadow_w = 0, drain_w = 0, polar_w = 0;
    for (int i = 0; i < FGLS_SHADOW_COUNT; i++)
        shadow_w += (b.meta.shadow_state[i] & 0x01u) ? 1 : 0;
    for (int i = 0; i < FGLS_DRAIN_COUNT; i++)
        drain_w  += (b.meta.drain_state[i]  & 0x01u) ? 2 : 0;
    for (int i = 0; i < FGLS_POLAR_COUNT; i++)
        polar_w  += (b.meta.polar_state[i]  & 0x01u) ? 1 : 0;
    int total = shadow_w + drain_w + polar_w;

    CHECK("A11a: weighted sum == 54 exact",   total == 54);
    CHECK("A11b: gp_blk_reshape_ready fires", gp_blk_reshape_ready(&b) == 1);

    /* partial fill — one drain missing → sum=52, should NOT fire */
    b.meta.drain_state[0] = 0;
    CHECK("A11c: partial fill (52) does NOT fire", gp_blk_reshape_ready(&b) == 0);
}
