 ...
 *
 *   diamond_slot = tile_id % FGLS_DIAMOND_COUNT   (54 slots)
 *   drain_idx    = tile_id % FGLS_DRAIN_COUNT      (12 drains = pentagons)
                                    ...
                                    const uint8_t chunk[64])
{
    ...
    uint8_t shadow_idx = (uint8_t)((s.drain_idx * 2u + a.dim) % FGLS_SHADOW_COUNT); /* %27 = 3³ */
...
/*
 * gp_blk_reshape_ready — weighted occupancy check.
 * threshold: shadow(27×1) + drain(12×2) + polar(3×1) = 54 exact
 *   shadow weight=1 : trit-cube slot (3³ family)
 *   drain  weight=2 : pentagon anchor (heavier — convergence point)
 *   polar  weight=1 : icosa pole N/S/C
 * Returns 1 if weighted sum >= FGLS_DIAMOND_COUNT (54).
 * O(27+12+3) = O(1).
 */
static inline int gp_blk_reshape_ready(const FrustumBlock *blk)
{
    int count = 0;
    for (int i = 0; i < FGLS_SHADOW_COUNT; i++)       /* 27×1 */
        count += (blk->meta.shadow_state[i] & 0x01u) ? 1 : 0;
    for (int i = 0; i < FGLS_DRAIN_COUNT; i++)        /* 12×2 */
        count += (blk->meta.drain_state[i]  & 0x01u) ? 2 : 0;
    for (int i = 0; i < FGLS_POLAR_COUNT; i++)        /*  3×1 */
        count += (blk->meta.polar_state[i]  & 0x01u) ? 1 : 0;
    return (count >= (int)FGLS_DIAMOND_COUNT) ? 1 : 0; /* 54 */
}
