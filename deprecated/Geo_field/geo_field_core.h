                                          ...
                                          GeoFieldEncodeStats *stats)
{
    ...
    uint8_t drain_idx = (uint8_t)(gp_is_pentagon(a.tile_id)
                      ? (a.tile_id % FGLS_DRAIN_COUNT)
                      : (gp_tile_to_pent(gf->gp_level, a.tile_id) % FGLS_DRAIN_COUNT));
    uint8_t shadow_idx = (uint8_t)((drain_idx * 2u + a.dim) % FGLS_SHADOW_COUNT); /* %27=3³ */
    ...
    memcpy(blk->data + (size_t)diamond_slot * FGLS_DIAMOND_BYTES,
           chunk, FGLS_DIAMOND_BYTES);
