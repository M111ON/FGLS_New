...
static uint32_t bs_encode_block(uint8_t *out, const uint8_t chunk48[48]) {
    ...
    /* Find best rotation via fibo_intersect popcount + least non-zero tiebreak */
    uint8_t  best_rot = 0;
    int      best_pc  = -1;
    int      best_nz  = BS_CHUNK_SZ;
    ...
    for (uint8_t rot = 0; rot < BS_ROT_STATES; rot++) {
        ...
        uint64_t isect = bs_fibo_intersect(rotbuf);
    ...
    /* DENSE: zstd compress rotated 48B */
    out[0] = BS_FLAG_DENSE;
    size_t bound = ZSTD_compressBound(BS_CHUNK_SZ);
    size_t csz = ZSTD_compress(out + 6, bound, best_buf, BS_CHUNK_SZ, 3);

    if (ZSTD_isError(csz) || csz >= BS_CHUNK_SZ) {
        /* zstd can't beat raw — store uncompressed */
        memcpy(out + 6, best_buf, BS_CHUNK_SZ);
        csz = BS_CHUNK_SZ;
    }
...
static uint32_t bs_decode_block(uint8_t out48[48], const uint8_t *in, uint32_t *bytes_read) {
    ...
    if (flag == BS_FLAG_DENSE) {
        ...
        uint8_t rotbuf[BS_CHUNK_SZ];
        if (csz == BS_CHUNK_SZ) {
            memcpy(rotbuf, in + 6, BS_CHUNK_SZ);
        } else {
            size_t dsz = ZSTD_decompress(rotbuf, BS_CHUNK_SZ, in + 6, (size_t)csz);
            if (ZSTD_isError(dsz) || dsz != BS_CHUNK_SZ) { memset(out48, 0, BS_CHUNK_SZ); *bytes_read = 0; return 0; }
        }
        bs_inv_rotate48(out48, rotbuf, rot);
        *bytes_read = 6 + csz;
        return BS_CHUNK_SZ;
