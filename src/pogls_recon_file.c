...
int pogls_recon_file_open(ReconContext *rc, const char *path)
{
    ...
    /* cache file size */
    if (file_seek64(rc->fp, 0, SEEK_END) != 0) { fclose(rc->fp); return RECON_ERR_IO; }
    {
        int64_t fs = file_tell64(rc->fp);
        if (fs < 0) { fclose(rc->fp); return RECON_ERR_IO; }
        rc->file_size = (uint64_t)fs;
    }
    if (file_seek64(rc->fp, 0, SEEK_SET) != 0) { fclose(rc->fp); return RECON_ERR_IO; }
                         ...
                         uint64_t      size)
{
    ...
    if (file_seek64(rc->fp, (int64_t)offset, SEEK_SET) != 0) return RECON_ERR_IO;
