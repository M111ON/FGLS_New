...
# ── Namespace encoding (prefix weight type into zone range) ────
NAMESPACE_SHIFT = {'Q': 12, 'K': 24, 'V': 36, 'O': 48,
                   'G': 60, 'U': 72, 'D': 84}

# ── Key encoding ──────────────────────────────────────────────
def _key(zone: int, shape: str, ns: str = None) -> tuple:
    assert 0 <= zone < N_ZONES, f"zone {zone} out of range"
    if ns is not None:
        assert ns in NAMESPACE_SHIFT, f"ns '{ns}' not in {list(NAMESPACE_SHIFT)}"
        zone += NAMESPACE_SHIFT[ns]
...
class GeometryStore:
    ...
    def __init__(self, path: str, read_only: bool = False):
        ...
        self._mmap_np: Optional[np.ndarray] = None   # cached read
        self._dat_fh = None
        self._query_cache: dict = {}    # {(zone, shape_idx): ndarray}
    ...
    def index(self, zone: int, shape: str, values: np.ndarray, ns: str = None):
        """
        ...
        ns: optional weight-type namespace ('Q','K','V','O','G','U','D')
        """
        assert not self.read_only, "Store opened read-only"
        k = _key(zone, shape, ns=ns)
    ...
    def query(self, zone: int, shape: str, ns: str = None) -> Optional[np.ndarray]:
        """
        ...
        arr = np.frombuffer(raw, dtype=np.float32).reshape(n_rows, n_cols).copy()
        self._query_cache[k] = arr
        return arr

    def query_batch(self, keys: list[tuple]) -> dict:
        """
        Batch query: [(zone, shape), ...] → {key: ndarray}
        Keys can be (zone, shape) without namespace, or (zone, shape, ns) for namespaced.
    ...
    def has(self, zone: int, shape: str, ns: str = None) -> bool:
        return _key(zone, shape, ns=ns) in self._index
    ...
    for ti, t in enumerate(tensors):
        ...
        for bi in range(0, x.shape[0], 64):
            ...
            for mode in range(4):   # ORBITAL, CHIRAL, CROSS, HUB
                ...
                real = v.real_mask
                zones  = v.zone[real].cpu().numpy()
                shapes = v.shape[real].cpu().numpy()
                vals   = batch[real].detach().cpu().numpy()

                # Group by (zone, shape) and write
                for ei in range(len(zones)):
                    z = int(zones[ei])
                    s = chr(int(shapes[ei]))
