...
/* Orphan layer — rotation + drain + fence + atomic reshape (Task #1-6) */
...
/* Goldberg shutter — archived (not used in DGLS) */
...
#define GF_BLOCK_DATA_SZ    DGLS_DATA_BYTES  /* 3456 = 54×64B           */
#define GF_BLOCK_TOTAL_SZ   DGLS_TOTAL_BYTES /* 4896 = full FrustumBlock */
                                          ...
                                          GeoFieldEncodeStats *stats)
{
    ...
    /* Derive indices */
    uint8_t diamond_slot = (uint8_t)(a.tile_id % DGLS_DIAMOND_COUNT);
    uint8_t drain_idx = (uint8_t)(gp_is_pentagon(a.tile_id)
                      ? (a.tile_id % DGLS_DRAIN_COUNT)
                      : (gp_tile_to_pent(gf->gp_level, a.tile_id) % DGLS_DRAIN_COUNT));
    uint8_t shadow_idx = (uint8_t)((drain_idx * 2u + a.dim) & 0x1Fu);

    /* PATH A: heptagon fence write gate (neutral key=0).
     * Fence is advisory on store path: CORE layer writes always proceed.
     * FENCE_DENY_LOCKED only increments contention counter -- no data lost,
     * no retry, no goto.  Mandatory gating is PATH B (drain/flush) only. */
    FenceResult fr = heptagon_fence_write(blk, shadow_idx,
                                           POGLS_LAYER_CORE, 0u, 0u);
    if (fr != FENCE_OK && stats)
        stats->zone_resets++;   /* contention counter */

    /* Data write -- always proceeds on CORE layer */
    memcpy(blk->data + (size_t)diamond_slot * DGLS_DIAMOND_BYTES,
           chunk, DGLS_DIAMOND_BYTES);

    /* Mark drain active (store path: bit0 only, no flush_pending) */
    blk->meta.drain_state[drain_idx] |= DRAIN_BIT_ACTIVE;

    /* Atomic Reshape -- one check per write, no inline retry */
    if (reshape_collect(blk))
        pogls_atomic_reshape(blk, NULL);
                                          ...
                                          uint8_t         out[64])
{
    ...
    memcpy(out, blk->data + (size_t)diamond_slot * DGLS_DIAMOND_BYTES,
           GF_CHUNK_SZ);
    return 0;
}
                                         ...
                                         uint8_t             out_chunk[64])
{
    ...
    /* Use route to select routing strategy (not always HUB) */
    MetaDecision md;
    switch (route) {
        case META_ROUTE_ORBITAL: md.next_enc = meta_orbital(enc);          break;
        case META_ROUTE_CHIRAL:  md.next_enc = meta_cpair(enc);            break;
        case META_ROUTE_CROSS:   md.next_enc = meta_cross(enc);            break;
        case META_ROUTE_HUB:
        default:                 md = meta_route(enc, dst_face);           break;
    }
...
{
    if (!gf || tile_id >= gf->face_max) return 0;
    int count = 0;
    uint32_t blocks_per_layer = (gf->face_max + GF_CHUNKS_PER_BLOCK - 1)
                                / GF_CHUNKS_PER_BLOCK;
    uint32_t tile_group  = tile_id / GF_CHUNKS_PER_BLOCK;
    for (uint8_t d = 0; d < GP_MAX_DIM; d++) {
        uint32_t block_idx = (uint32_t)d * blocks_per_layer + tile_group;
        if (block_idx >= gf->n_blocks) break;
        GpAddr a = { .tile_id = tile_id, .dim = d };
        if (gp_blk_read(&gf->blocks[block_idx], gf->gp_level, a) != NULL)
            count++;
    }
    return count;
}
                                        ...
                                        void           *ctx)
{
    ...
    for (uint8_t d = 0; d < GP_MAX_DIM; d++) {
        ...
        if (block_idx >= gf->n_blocks) break;
        GpAddr a = { .tile_id = tile_id, .dim = d };
        const uint8_t *chunk = gp_blk_read(&gf->blocks[block_idx], gf->gp_level, a);
        if (chunk) {
...
} __attribute__((packed)) GeoFieldFileHeader;
