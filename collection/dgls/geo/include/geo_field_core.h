                                          ...
                                          GeoFieldEncodeStats *stats)
{
    ...
    memcpy(blk->data + (size_t)diamond_slot * DGLS_DIAMOND_BYTES, chunk, DGLS_DIAMOND_BYTES);
    ...
    blk->meta.drain_state[drain_idx % DGLS_DRAIN_COUNT] |= 0x01u;
     ...
     */
    uint8_t sec_drain = tring_spk % DGLS_DRAIN_COUNT;
    if (sec_drain != (drain_idx % DGLS_DRAIN_COUNT))
