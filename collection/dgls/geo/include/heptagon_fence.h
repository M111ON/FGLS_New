                                                ...
                                                uint8_t       phase)
{
    ...
    uint8_t     *sb = &m->shadow_state[shadow_id % DGLS_SHADOW_COUNT];
...
static inline bool heptagon_slot_locked(const FrustumBlock *block,
                                         uint8_t shadow_id)
{
    return (bool)(block->meta.shadow_state[shadow_id % DGLS_SHADOW_COUNT]
                  & SHADOW_BIT_FENCE);
}
                                         ...
                                         uint8_t shadow_id)
{
    return _fence_key_read(
        block->meta.shadow_state[shadow_id % DGLS_SHADOW_COUNT]);
}
