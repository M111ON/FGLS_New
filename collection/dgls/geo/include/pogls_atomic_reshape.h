...
typedef struct {
    ...
    uint8_t  drain_count;       /* always DGLS_DRAIN_COUNT = 12          */
} ReshapeStage;
                                           ...
                                           ReshapeStage       *out)
{
    ...
    out->drain_count     = (uint8_t)DGLS_DRAIN_COUNT;
...
static inline ReshapeResult reshape_drain(FrustumBlock *block)
{
    ...
    for (uint8_t d = 0u; d < (uint8_t)DGLS_DRAIN_COUNT; d++) {
    ...
    if (fail_count == (uint8_t)DGLS_DRAIN_COUNT)
 ...
 * Returns value in [0..28]. Threshold fires at 54 (> DGLS_SHADOW_COUNT=28).
 *
 * Note: DGLS_SHADOW_COUNT=28 < POGLS_SACRED_NEXUS=54 — this means
