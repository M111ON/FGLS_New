 ...
 *
 *  Drain ID selection:
 *    drain_id = trit % DGLS_DRAIN_COUNT   (0..11 — 12 pentagon drains)
                                             ...
                                             PoglsLayer    src_layer)
{
    ...
    const uint8_t drain_id  = trit % DGLS_DRAIN_COUNT;          /* 0..11 */
    const uint8_t shadow_id = drain_id % DGLS_SHADOW_COUNT;     /* 0..27 */
...
static inline uint8_t fabric_shadow_flush_pending(const FrustumBlock *block)
{
    ...
    for (uint8_t i = 0u; i < DGLS_SHADOW_COUNT; ++i) {
