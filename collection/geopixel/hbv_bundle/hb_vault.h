        ...
        const char  *out_path)
{
    ...
    /* header[0..7]  = orig_sz64  (uncompressed size, LE uint64) */
    /* header[8..15] = zdata_sz64 (compressed data size, LE uint64) */
...
static inline int hbv_unpack(const char *vault_path, const char *out_dir, int verify_sha)
{
    ...
    if (zdata_sz64 == 0 || zdata_sz64 > zstream_sz - 16){ free(zstream); return HB_ERR_TILES; }
