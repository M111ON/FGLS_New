...
typedef struct {
    ...
    /* O1: track which tile slots belong to TEMP lane (64-slot bitmap) */
    uint64_t   temp_slots;  /* bit N set = slot N is TEMP lane */
    uint32_t   feed_idx;    /* current slot index (resets each flush) */
...
static void flush_packet(PipeState *st) {
    ...
    for (uint32_t t = 0; t < n_tiles; t++) {
        ...
        /* O1: select codec by lane — MAIN=SEED, TEMP=FREQ(+ZSTD19) */
        int is_temp = (t < 64u) && ((st->temp_slots >> t) & 1u);
        uint8_t codec = is_temp ? GPX5_CODEC_FREQ : GPX5_CODEC_SEED;
    ...
    st->temp_slots = 0u;   /* reset TEMP bitmap */
    st->feed_idx   = 0u;
}
...
static void on_chunk(const ScanEntry *e, void *user) {
    ...
    if (lane == 1) {
        ...
        st->path_temp++;
        st->n_temp++;
        /* O1: mark this slot as TEMP for codec routing in flush */
        if (st->feed_idx < 64u)
            st->temp_slots |= (1ull << st->feed_idx);
    }
