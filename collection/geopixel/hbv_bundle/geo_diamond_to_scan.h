...
#ifndef SCAN_ENTRY_DEFINED
#define SCAN_ENTRY_DEFINED

typedef struct __attribute__((packed)) {
    uint8_t face;   /* 0..11 dodecahedron face  */
    uint8_t edge;   /* 0..4  face edge          */
    uint8_t z;      /* 0..255 depth             */
} ThetaCoord;

typedef struct {
    uint64_t   seed;        /* XOR-fold fingerprint                     */
    ThetaCoord coord;       /* geometric address                        */
    uint8_t    flags;       /* SCAN_FLAG_*                              */
    uint8_t    field_id;    /* FHW_FIELD_* (0..3)                       */
    uint8_t    _pad;
    uint32_t   chunk_idx;   /* position in stream (hop_count)           */
    uint32_t   checksum;    /* lower 32b of seed for quick verify       */
} ScanEntry;               /* 20B */
