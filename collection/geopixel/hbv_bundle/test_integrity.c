...
static void test_byte_exact(uint8_t *data, uint64_t N){
    ...
    /* Copy encode stats (n_seed..bytes_enc) WITHOUT overwriting dec_ok/dec_fail */
    memcpy(&rd.n_seed,&r.n_seed,(uintptr_t)&r.dec_ok-(uintptr_t)&r.n_seed);
...
static void test_cross_file(uint8_t *dataA, uint64_t NA,
    ...
    memcpy(&rd.n_seed,&re.n_seed,(uintptr_t)&re.dec_ok-(uintptr_t)&re.n_seed);
