...
int main(void) {
    FILE *fin = fopen("test_input.bmp", "rb");
    if (!fin) { fprintf(stderr, "cannot open test_input.bmp\n"); return 1; }
    fseek(fin, 0, SEEK_END); long fsz = ftell(fin); fseek(fin, 0, SEEK_SET);
    uint8_t *buf = malloc((size_t)fsz);
    if (fread(buf, 1, (size_t)fsz, fin) != (size_t)fsz) { fclose(fin); free(buf); return 1; }
    fclose(fin);
    ...
    FILE *fout = fopen("test_output.seq", "wb");
    if (!fout) { fprintf(stderr, "cannot create test_output.seq\n"); free(buf); return 1; }

    /* ── Write HbHeaderFrame (cover: 64B) ──────────────────────────── */
    HbHeaderFrame hdr_frame;
    memset(&hdr_frame, 0, sizeof(hdr_frame));
    hdr_frame.magic        = HBHF_MAGIC;
    hdr_frame.version      = HBHF_VERSION;
    hdr_frame.n_cycles     = 1;                    /* single cycle file */
    hdr_frame.n_layers     = 1;                    /* Y layer only */
    hdr_frame.total_tiles  = (uint32_t)n_chunks;   /* one tile per chunk */
    hdr_frame.img_w        = 0;                    /* not an image */
    hdr_frame.img_h        = 0;
    hdr_frame.tile_w       = 8;                    /* GpxTileInput is 8x8 */
    hdr_frame.tile_h       = 8;
    hdr_frame.global_seed  = first_seed;
    hdr_frame.tick_period  = 144;                  /* Fibonacci closure */
    for (int s = 0; s < 8; s++) hdr_frame.codec_map[s] = 0; /* raw */
    hdr_frame.layer_stride = 192;                  /* RGB tile bytes */
    hdr_frame.crc32        = 0;                    /* will be set below */

    uint8_t hdr_buf[HBHF_SZ];
    hbhf_write(hdr_buf, &hdr_frame);               /* serialise to 64B */
    if (fwrite(hdr_buf, 1, HBHF_SZ, fout) != HBHF_SZ) {
        fprintf(stderr, "failed to write header frame\n");
        fclose(fout); free(buf); return 1;
    }
    printf("wrote HbHeaderFrame: %u B\n", HBHF_SZ);

    /* ── Write FiboLayerHeader (32B) ─────────────────────────────── */
    FiboLayerHeader fibo_hdr;
    memset(&fibo_hdr, 0, sizeof(fibo_hdr));
    if (!fibo_layer_header_init(&fibo_hdr, 8u, 0u, first_seed)) {
        fprintf(stderr, "fibo_layer_header_init failed\n");
        fclose(fout); free(buf); return 1;
    }
    if (fwrite(&fibo_hdr, 1, sizeof(fibo_hdr), fout) != sizeof(fibo_hdr)) {
        fprintf(stderr, "failed to write fibo layer header\n");
        fclose(fout); free(buf); return 1;
    }
    printf("wrote FiboLayerHeader: %zu B\n", sizeof(fibo_hdr));

    /* ── Process each chunk → tile ────────────────────────────────── */
    printf("\n── Processing chunks → Geopixel sequence ──\n");
    ...
    for (uint64_t ci = 0; ci < n_chunks; ci++) {
        ...
        /* ── Write tile to sequence file ─────────────────────────── */
        if (tile_cost == 192) {
            /* write full RGB tile */
            if (fwrite(tile.tile_rgb, 1, FTD_TILE_BYTES, fout) != FTD_TILE_BYTES) {
                fprintf(stderr, "failed to write tile %llu\n", (unsigned long long)n_tiles);
                break;
            }
        } else {
            /* write seed-only: route_id (8B) + simple checksum */
            uint8_t seed_buf[8];
            memcpy(seed_buf, &tile.route_id, 8);
            if (fwrite(seed_buf, 1, 8, fout) != 8) {
                fprintf(stderr, "failed to write seed for tile %llu\n", (unsigned long long)n_tiles);
                break;
            }
        }
    ...
    fclose(fout);
    printf("\nwrote sequence file: test_output.seq\n");
    ...
    printf("\n── Storage breakdown ──\n");
    uint32_t cover_cost = HBHF_SZ + sizeof(FiboLayerHeader);  /* 64 + 32 */
