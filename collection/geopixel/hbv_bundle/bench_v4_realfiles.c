/*
 * bench_v4_realfiles.c — Test v4 pipeline on real file types
 * Build: gcc -O2 -I. -Icore/twin_core bench_v4_realfiles.c -lm -o bench_v4_realfiles
 */
#include <stdint.h>
#include <string.h>
#include <stdio.h>
...
#define PASSTHRU_ENTROPY 20
#define PASSTHRU_ISECT   2
...
static uint8_t d4_canon_rid(const uint8_t s[64],uint8_t out[64]){
    ...
    for(int i=0;i<8;i++){uint8_t tmp[64];d4_apply(tmp,s,i);
        uint64_t th=fnv64(tmp,64);if(th<bh){bh=th;bi=i;memcpy(out,tmp,64);}}
    return (uint8_t)bi;
}
...
static SeedIndex* sx(){SeedIndex*x=calloc(1,sizeof(SeedIndex));x->slots=calloc(SIDX_CAP,sizeof(SeedSlot));return x;}
...
static void sp(SeedIndex*x,uint64_t seed,const uint8_t*ch,uint8_t rid){
    ...
    for(int i=0;i<SIDX_CAP;i++){uint32_t s=(s0+i)&SIDX_MASK;
        if(!x->slots[s].used){x->slots[s].used=1;x->slots[s].seed=seed;x->slots[s].rid=rid;memcpy(x->slots[s].chunk,ch,64);x->ne++;return;}
...
static uint64_t dseed(const uint8_t c[64]){
    ...
    s^=s>>33;s*=0xff51afd7ed558ccdULL;s^=s>>33;s*=0xc4ceb9fe1a85ec53ULL;s^=s>>33;return s;}
...
static uint64_t chunk_isect_cached(const uint8_t *chunk, DiamondBlock *db_out){
    uint8_t face,edge,z;uint64_t seed=dseed(chunk);dcoord(seed,&face,&edge,&z);
    *db_out=fold_block_init(face,edge,(uint32_t)z<<16,1,0);
    memcpy(&db_out->core.raw,chunk,8);db_out->invert=~db_out->core.raw;fold_build_quad_mirror(db_out);
    return fold_fibo_intersect(db_out);}
static int chunk_entropy(const uint8_t *c){uint32_t seen=0;for(int i=0;i<64;i++)seen|=(1u<<c[i]);return __builtin_popcount(seen);}

static int lz77_compress(const uint8_t *src, uint8_t *dst){
    int di=0,si=0,dec=0;uint8_t decbuf[64];memset(decbuf,0,64);
    while(si<64 && di<118){
        ...
        if(best_len>=LZ_MIN_MATCH){
            if(di+3>=118)break;
            dst[1+di++]=0xFF;dst[1+di++]=(uint8_t)(best_len-LZ_MIN_MATCH);dst[1+di++]=(uint8_t)best_off;
            for(int k=0;k<best_len;k++)decbuf[dec+k]=src[si+k];dec+=best_len;si+=best_len;
        }else{
            if(di+2>=118)break;
            /* escape 0xFE and 0xFF */
            if(src[si]>=0xFE){dst[1+di++]=0xFE;dst[1+di++]=src[si];}
            else{dst[1+di++]=src[si];}
            decbuf[dec++]=src[si++];}}
    if(di>=64){dst[0]=65;memcpy(dst+1,src,64);return 65;}
    dst[0]=(uint8_t)(1+di);return 1+di;}

static int lz77_decompress(const uint8_t *src, uint8_t *dst){
    int clen=src[0],si=1,di=0;
    while(si<clen && di<64){
        if(src[si]==0xFF){int len=LZ_MIN_MATCH+src[si+1],off=src[si+2];si+=3;
            for(int i=0;i<len && di<64;i++){dst[di]=dst[di-off];di++;}}
        else if(src[si]==0xFE){dst[di++]=src[si+1];si+=2;}
        else{dst[di++]=src[si++];}}
    return di;}
...
typedef struct{double ratio;double pct;double enc_ms,dec_ms;
    uint64_t n,ok,ns,nd,nl,nda,nlz,raw,enc,geom,flow_resets,anti_fake;}Metrics;
...
static Metrics run_pipe(const uint8_t*data,uint64_t N,SeedIndex*idx){
    ...
    for(uint64_t i=0;i<N;i++){
        ...
        DiamondBlock db;uint64_t isect=chunk_isect_cached(c,&db);
        int isect_pc=(int)__builtin_popcountll(isect);
        int has_geom=(isect_pc>=GEOM_THRESHOLD);int ent=chunk_entropy(c);
        if(has_geom)m.geom++;
        if(isect==0 && flow.hop_count>0){m.flow_resets++;diamond_flow_init(&flow);}

        /* Gate D: LZ passthru */
        if(ent>PASSTHRU_ENTROPY && isect_pc<PASSTHRU_ISECT){
            uint8_t lzbuf[128];int clen=lz77_compress(c,lzbuf);
            if(clen<64){enc[i].flag=4;enc[i].rid=(uint8_t)clen;memcpy(enc[i].r,lzbuf,clen);
                m.nlz++;m.enc+=1+clen;continue;}}

        if(!has_geom){
            uint8_t cn[64],hp[64];uint8_t rd=d4_canon_rid(c,cn);h_fwd(hp,cn);
            enc[i].flag=2;enc[i].rid=rd;memcpy(enc[i].r,hp,64);m.nl++;m.enc+=65;
            ...
            flow.route_addr=diamond_route_update(flow.route_addr,isect);flow.hop_count++;
            m.ns++;m.enc+=9;
        }else{
            flow.route_addr=diamond_route_update(flow.route_addr,isect);flow.hop_count++;
            FlowEndReason reason=diamond_flow_end(&db,bl,isect,flow.route_addr,flow.hop_count,flow.drift_acc);
            if(reason!=FLOW_CONTINUE){
                ...
                uint8_t cn[64],hp[64];uint8_t rd=d4_canon_rid(c,cn);h_fwd(hp,cn);
                enc[i].flag=1;enc[i].rid=rd;memcpy(enc[i].r,hp,64);m.nd++;m.enc+=17;}}}
    ...
    for(uint64_t i=0;i<N;i++){
        ...
        }else if(enc[i].flag==4){lz77_decompress(enc[i].r,dec+i*64);}
    ...
    m.ratio=m.enc>0?(double)m.raw/(double)m.enc:0;m.pct=m.n>0?(double)m.ok*100.0/(double)m.n:0;
...
static void bench_one(const char *label, const uint8_t *data, uint64_t len){
    ...
    printf("%-12s ",label);
    printf("N=%5llu  ratio=%5.3fx  seed=%3.0f%%  lz=%3.0f%%  exact=%4.1f%%\n",
           (unsigned long long)N,m.ratio,(double)m.ns*100.0/(double)N,(double)m.nlz*100.0/(double)N,m.pct);

    /* Shuffle test */
    uint8_t *shuf=malloc(N*64);
    for(uint64_t i=0;i<N;i++){uint64_t j=i+(uint64_t)rand()%(N-i);
        memcpy(shuf,buf+i*64,64);memcpy(buf+i*64,buf+j*64,64);memcpy(buf+j*64,shuf,64);}
    free(shuf);
    SeedIndex *idx2=sx();Metrics m2=run_pipe(buf,N,idx2);
...
static uint8_t *load_file(const char *path, uint32_t *len_out){
    ...
    uint8_t *buf=malloc((size_t)sz);if(fread(buf,1,(size_t)sz,f)!=(size_t)sz){fclose(f);free(buf);return NULL;}
    fclose(f);*len_out=(uint32_t)sz;return buf;}

int main(void){
    init_h();init_d4();srand(42);
    printf("╔══════════════════════════════════════════════════╗\n");
    printf("║   V4 Pipeline Benchmark — Real Files             ║\n");
    printf("║   Gate: isect (G=%d S=%d)  LZ: ent>%d isect<%d ║\n",
           GEOM_THRESHOLD,SEED_POPCNT_MIN,20,PASSTHRU_ISECT);
    printf("╚══════════════════════════════════════════════════╝\n\n");
