 ...
 * test_integrity_v2.c
 * Changes from _fixed:
 *   A. fold_xor_audit replaced with content-based geometric gate:
 *      chunk passes geometry path if popcnt(isect) >= GEOM_THRESHOLD
 *      (isect = fibo_intersect of chunk's own 4-way rotation)
 *      → audit now reflects chunk data, not a self-fulfilling invariant
 *
 *   B. flow boundary is content-driven, not position-driven:
 *      flow resets when isect==0 (natural dead zone)
 *      flow_active removed — replaced by ctx.hop_count > 0
 *      agg_isect = accumulated route_addr (content fingerprint, not index)
 *
 *   C. entropy gate uses isect quality, not raw unique-byte count:
 *      high isect popcnt → seed path (geometric structure found)
 *      low isect popcnt  → lossless (no structure to exploit)
 *
 * Build: gcc -O2 -I. test_integrity_v2.c -lm -o test_v2
 */
#include <stdint.h>
#include <string.h>
#include <stdio.h>
...
#define FLOW_DEAD_BITS   0    /* isect popcount ≤ this = natural boundary */
 ...
 * isect = bits ที่คงอยู่ทุก rotation = geometric fingerprint
 * ไม่ใช้ XOR audit (มัน trivially true เสมอ)
 */
static uint64_t chunk_isect(const uint8_t *chunk){
    uint8_t face,edge,z;
...
typedef struct {
    uint64_t n;
    uint64_t n_seed,n_delta,n_lossless,n_dna,n_passthru;
    uint64_t bytes_raw,bytes_enc;
    uint64_t dec_ok;
    double   enc_ms,dec_ms;
    /* diagnostic */
    uint64_t geom_chunks;  /* chunks with isect >= GEOM_THRESHOLD */
    uint64_t flow_resets;  /* natural boundary hits */
} Result;

/* helper: encode one chunk via D4+Hilbert → seed index */
                           ...
                           SeedIndex *sidx, Encoded **enc_out)
{
    ...
    *dec_out=calloc(N,64);
    ...
    for(uint64_t i=0;i<N;i++){
        ...
        /* ── Gate A: geometric quality of this chunk ── */
        uint64_t isect=chunk_isect(chunk);
        ...
        if(isect==0 && flow.hop_count>0){
            /* natural dead zone → close current flow with DNA marker */
            r.flow_resets++;
        ...
        /* ── Dispatch ── */
        if(!has_geom){
            /* no geometric structure → lossless, no flow involvement */
            uint8_t canon[64]; uint8_t rid=d4_canon_rid(chunk,canon);
            uint8_t hperm[64]; h_fwd(hperm,canon);
            ...
            /* strong geometry → seed dedup (most common path for text) */
            enc_via_seed(&enc[i],sidx,chunk,0);
            /* accumulate flow fingerprint */
            ...
            uint8_t face,edge,z;
            uint64_t seed2=derive_seed(chunk);
            derive_coord(seed2,&face,&edge,&z);
            DiamondBlock db=fold_block_init(face,edge,(uint32_t)z<<16,1,0);
            memcpy(&db.core.raw,chunk,8);
            db.invert=~db.core.raw;
            fold_build_quad_mirror(&db);
            ...
            if(reason!=FLOW_CONTINUE){
                ...
                /* delta: store hperm inline (no seed lookup needed) */
                uint8_t canon[64]; uint8_t rid=d4_canon_rid(chunk,canon);
                uint8_t hperm[64]; h_fwd(hperm,canon);
...
/* ════════════════════════════════════════════════════════════════
   decode_all — unchanged logic, same as _fixed
   ════════════════════════════════════════════════════════════════ */
...
static void test_byte_exact(const uint8_t *data, uint64_t N){
    ...
    dr.n=er.n; dr.n_seed=er.n_seed; dr.n_dna=er.n_dna;
    dr.n_delta=er.n_delta; dr.n_lossless=er.n_lossless;
    dr.geom_chunks=er.geom_chunks; dr.flow_resets=er.flow_resets;
    dr.bytes_enc=er.bytes_enc; dr.enc_ms=er.enc_ms;
    print_result(&er,&dr,"v2 geometry pipeline");
...
static void test_shuffle(uint8_t *data, uint64_t N){
    ...
    printf("  ordered  : ratio=%.3fx  byte-exact=%llu/%llu  flow_resets=%llu\n",
           ratio1,(unsigned long long)dr1.dec_ok,(unsigned long long)N,
           (unsigned long long)r1.flow_resets);
    printf("  shuffled : ratio=%.3fx  byte-exact=%llu/%llu  flow_resets=%llu\n",
           ratio2,(unsigned long long)dr2.dec_ok,(unsigned long long)N,
           (unsigned long long)r2.flow_resets);
...
/* ── isect distribution probe — show what GEOM_THRESHOLD does ── */
static void probe_isect(const uint8_t *data, uint64_t N){
    printf("\n═══ PROBE: isect popcnt distribution ═══\n");
    int buckets[65]={0};
    for(uint64_t i=0;i<N;i++){
        uint64_t isc=chunk_isect(data+i*64);
...
int main(void){
    ...
    printf("sizeof(Encoded)=%zu  GEOM_THRESHOLD=%d  SEED_POPCNT_MIN=%d\n\n",
