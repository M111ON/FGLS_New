...
/* One content-driven segment */
typedef struct {
    uint32_t   offset;   /* byte offset in original data */
    uint32_t   length;   /* byte length of this segment (1..max_chunk) */
} FlowSegment;
...
static inline int flow_chunk(const uint8_t *data, size_t data_sz,
    ...
    while (pos < data_sz) {
        uint32_t remaining = (uint32_t)(data_sz - pos);
        uint32_t chunk_end = pos + (remaining < max_chunk ? remaining : max_chunk);

        /* Find a dead zone boundary within [pos + min_chunk, chunk_end] */
        uint32_t boundary = data_sz; /* default: end of file */
        uint32_t scan_start = pos + min_chunk;
        if (scan_start > data_sz) scan_start = (uint32_t)data_sz;

        for (uint32_t bp = scan_start; bp <= chunk_end && bp + FLOW_WINDOW <= data_sz; bp++) {
            int isect = flow_isect_at(data, data_sz, bp);
