/*
 * diamond_geo_pipe_v6.c — Flow-zone + transform sequence pipeline
 *
 * Flow chunker splits input at geometric dead zones (isect==0).
 * Each zone has its own transform sequence state (no cross-zone transforms).
 * Within a zone, 64B sub-chunks use ts_encode + smart_encode.
 *
 * Container:
 *   [onion_header:64B]
 *   [n_zones:4B]
 *   [zone_record:n_zones * 8B]  — each: {chunk_count:4B, first_seq:4B}
 *   [n_total_chunks:4B]
 *   [strategy_byte + payload...] per chunk
 *
 * Each zone's chunks are Hilbert-traversed globally but tagged so
 * transforms reset at zone boundaries.
 */
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
...
typedef struct {
    ...
    int           n_chunks;
    uint32_t      written;
    /* Per-zone encode state (reset at zone boundary) */
    TsEncCtx     *ts_ctx;      /* one per zone */
    DiamondField *df;          /* shared tring */
    int           cur_zone;    /* current zone being encoded */
    int           n_zones;
    ZoneRecord   *zones;       /* zone descriptors */
    uint32_t      zone_first_seq; /* seq_pos at zone start */
} EncTraverseCtx;

static int enc_traverse(void *ctx, uint32_t ci, uint32_t seq_pos){
    EncTraverseCtx *e = (EncTraverseCtx*)ctx;

    /* Check if this chunk starts a new zone */
    int new_zone = 0;
    for (int z = 0; z < e->n_zones; z++) {
        if (ci >= e->zones[z].chunk_offset &&
            ...
            new_zone = 1;
            /* Reset transform state for new zone */
    ...
    /* Encode using current zone's ts_ctx */
    TsEncCtx *ts = &e->ts_ctx[e->cur_zone];
    uint32_t esz;
    const uint8_t *enc = ts_encode(ts, chunk, &esz, seq_pos);
...
static int do_encode(const uint8_t *data, size_t orig_size,
    ...
    for (int i = 0; i < n_chunks; i++) {
        uint8_t s = ts_ctx[0].base.stored_enc[i][0];
        for (int z = 1; z < n_zones; z++) {
            if (ts_ctx[z].base.stored_enc[i]) {
                s = ts_ctx[z].base.stored_enc[i][0];
                break;
            }
        }
        if (s < 10) { cnt[s]++; bytes[s] += 0; } /* bytes tracked per-zone */
    }
    ...
    printf("v6_flow  orig=%zuB chunks=%d enc=%uB ratio=%.2fx [%.1fms] zones=%d\n",
