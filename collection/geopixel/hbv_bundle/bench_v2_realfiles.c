/*
 * bench_v2_realfiles.c — Test v2 pipeline on real file types
 * Build: gcc -O2 -I. -Icore/twin_core bench_v2_realfiles.c -lm -o bench_v2_realfiles
 */
#include <stdint.h>
#include <string.h>
#include <stdio.h>
...
typedef struct{double ratio;double pct;double enc_ms,dec_ms;
    uint64_t n,ok,ns,nd,nl,nda,raw,enc,geom,flow_resets;}Metrics;
...
static Metrics run_pipe(const uint8_t*data,uint64_t N,SeedIndex*idx){
    ...
    for(uint64_t i=0;i<N;i++){
        ...
        if(!has_geom){
            ...
            uint8_t face,edge,z;
            uint64_t seed2=dseed(c);dcoord(seed2,&face,&edge,&z);
            DiamondBlock db=fold_block_init(face,edge,(uint32_t)z<<16,1,0);
...
static void bench_one(const char *label, const uint8_t *data, uint64_t len){
    ...
    printf("%-12s ", label);
    printf("N=%5llu  ratio=%5.3fx  geom=%3.0f%%  seed=%3.0f%%  exact=%4.1f%%  resets=%llu\n",
    ...
    printf("  └─ shuffled               ratio=%5.3fx  drop=%+.1f%%  resets=%llu\n",
           m2.ratio, drop, (unsigned long long)m2.flow_resets);
...
int main(void){
    ...
    printf("╔══════════════════════════════════════════════════╗\n");
    printf("║   V2 Pipeline Benchmark — Real Files             ║\n");
    printf("║   Gate: isect popcnt (G=%d S=%d)                ║\n",
