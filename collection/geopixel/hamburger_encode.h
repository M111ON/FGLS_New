    ...
    case GPX5_CODEC_RICE3: {
        ...
        for (uint32_t i = 0; i < in_sz; i++) {
            ...
            if ((bit_pos >> 3) >= out_cap - 4u) {
                /* overflow: fall back to raw with orig_sz header intact */
                if (in_sz + 2u > out_cap) return 0u;
                g5w2(out, (uint16_t)in_sz);
                memcpy(out + 2u, in, in_sz);
                return in_sz + 2u;
            }
        }
...
static inline int hb_encode_run(
        ...
        uint32_t n_tiles)
{
    if (n_tiles == 0) return HB_ERR_TILES;  /* multi-cycle: no upper limit */
        ...
        uint8_t       use_2carrier)
{
    ...
    if (n_tiles == 0) return HB_ERR_TILES;

    uint32_t tile_bytes = (uint32_t)(img->tw * img->th * 6u);
    if (tile_bytes > HB_TILE_SZ_MAX) return HB_ERR_TILES;

    /* ── classify ALL tiles once ── */
    uint8_t *ttypes = (uint8_t *)calloc(n_tiles, 1u);
    if (!ttypes) return HB_ERR_ALLOC;
    hb_classify_batch(img->Y, img->Cg, img->Co, img->w,
                      img->tw, img->th, tiles_x, tiles_y, ttypes);

    /* ── multi-cycle: split into chunks of HB_MAX_TILES ── */
    uint32_t n_cycles = (n_tiles + HB_MAX_TILES - 1u) / HB_MAX_TILES;
    int r = HB_OK;

    /* open file for first cycle (truncate), append for rest */
    for (uint32_t cyc = 0; cyc < n_cycles && r == HB_OK; cyc++) {
        uint32_t tile_start = cyc * HB_MAX_TILES;
        uint32_t tile_end   = tile_start + HB_MAX_TILES;
        if (tile_end > n_tiles) tile_end = n_tiles;
        uint32_t cyc_tiles  = tile_end - tile_start;

        /* cycle-local ttype slice for pipe selection */
        Gpx5PipeEntry pipes[3];
        hb_auto_pipes(ttypes + tile_start, cyc_tiles, pipes);

        uint8_t  *tile_pool = (uint8_t *)calloc(cyc_tiles, HB_TILE_SZ_MAX);
        HbTileIn *tiles     = (HbTileIn *)calloc(cyc_tiles, sizeof(HbTileIn));
        if (!tile_pool || !tiles) {
            free(tile_pool); free(tiles); r = HB_ERR_ALLOC; break;
        }

        for (uint32_t ci = 0; ci < cyc_tiles; ci++) {
            uint32_t ti = tile_start + ci;
            int tx = (int)(ti % (uint32_t)tiles_x);
            int ty = (int)(ti / (uint32_t)tiles_x);
            int x0 = tx * img->tw;
            int y0 = ty * img->th;
            uint8_t *dst = tile_pool + ci * HB_TILE_SZ_MAX;
            uint32_t off = 0u;
            ...
            tiles[ci].data    = dst;
            tiles[ci].sz      = off;
            tiles[ci].tile_id = (uint16_t)ci;   /* local id within cycle */
            tiles[ci].ttype   = ttypes[ti];
        }

        /* cycle path: "base.gpx5" → "base_c000.gpx5", "base_c001.gpx5" ... */
        char cyc_path[512];
        if (n_cycles == 1) {
            snprintf(cyc_path, sizeof(cyc_path), "%s", path);
        } else {
            /* strip extension, append _cNNN */
            const char *dot = strrchr(path, '.');
            if (dot) {
                size_t base_len = (size_t)(dot - path);
                snprintf(cyc_path, sizeof(cyc_path), "%.*s_c%03u%s",
                         (int)base_len, path, cyc, dot);
            } else {
                snprintf(cyc_path, sizeof(cyc_path), "%s_c%03u", path, cyc);
            }
        }

        /* cycle-local tiles_y = ceil(cyc_tiles / tiles_x) */
        uint16_t cyc_ty = (uint16_t)((cyc_tiles + (uint32_t)tiles_x - 1u) / (uint32_t)tiles_x);

        HbEncodeCtx ctx;
        hb_encode_init(&ctx, global_seed,
            (uint16_t)tiles_x, cyc_ty,
            pipes, use_2carrier);

        r = hamburger_encode(cyc_path, &ctx, tiles, cyc_tiles);
        fprintf(stderr, "[hb_encode_image] cycle %u/%u tiles=%u path=%s r=%d\n",
                cyc, n_cycles-1u, cyc_tiles, cyc_path, r);
    ...
    /* ── wire header frame into cycle-0 file (auto frame[0]) ── */
    {
        /* build codec_map: codec per shell from cycle-0 pipes */
        Gpx5File gf0;
        uint8_t codec_map[8] = {0};
        const char *c0path = path;  /* single-cycle uses path directly */
        char tmp_c0[512];
        if (n_cycles > 1) {
            const char *dot = strrchr(path, '.');
            if (dot) snprintf(tmp_c0, sizeof(tmp_c0), "%.*s_c000%s",
                              (int)(dot-path), path, dot);
            else     snprintf(tmp_c0, sizeof(tmp_c0), "%s_c000", path);
            c0path = tmp_c0;
        }
        if (gpx5_open(c0path, &gf0) == 0) {
            for (uint8_t sh = 0; sh < 8u; sh++) {
                uint32_t rep_tile = (uint32_t)sh * FSW_TICKS_PER_SHELL;
                codec_map[sh] = gf0.pipes[rep_tile % 3u].codec;
            }
            gpx5_close(&gf0);
        }
        ...
        hf.n_cycles     = (uint8_t)(n_cycles > 255u ? 255u : n_cycles);
        hf.n_layers     = 3u;   /* YCgCo */
        hf.total_tiles  = n_tiles;
        hf.img_w        = (uint16_t)img->w;
        hf.img_h        = (uint16_t)img->h;
        hf.tile_w       = (uint16_t)img->tw;
        hf.tile_h       = (uint16_t)img->th;
        hf.global_seed  = global_seed;
        hf.tick_period  = GPX5_TICK_PERIOD;
        memcpy(hf.codec_map, codec_map, 8u);
        hf.layer_stride = (uint32_t)(img->tw * img->th * 2u);  /* 2B per pixel */

        gpx5_append_hbhf(c0path, &hf);
    }
...
/* ══════════════════════════════════════════════════════════
 * HbFiboLut — extended O(1) random-access index
 *
 * Maps tile_id → fibo clock position so caller can seek
 * directly to any tile by tick without full decode.
 *
 * Layout (per tile, 12B):
 *   tile_id      2B  — local tile index within cycle
 *   cycle_id     2B  — which gpx5 cycle file
 *   tick         1B  — fibo clock phase (0..143)
 *   shell_n      1B  — cube shell (0..7)
 *   spoke        1B  — tring spoke (0..5)
 *   codec_id     1B  — GPX5_CODEC_*
 *   invert_off   4B  — byte offset into cycle file (copy from Gpx5LutEntry)
 * ══════════════════════════════════════════════════════════ */

#define HB_FIBO_LUT_ENTRY_SZ  12u

typedef struct {
    uint16_t tile_id;       /* local id within cycle                  */
    uint16_t cycle_id;      /* which cycle (0..n_cycles-1)            */
    uint8_t  tick;          /* fibo clock phase 0..143                */
    uint8_t  shell_n;       /* cube shell 0..7                        */
...
/*
 * hb_fibo_lut_build — build extended LUT from decoded Gpx5File
 *
 * cycle_id : which cycle this file belongs to
 * gf       : opened Gpx5File (lut already loaded)
 * out      : caller-alloc array of gf->hdr.n_tiles entries
 */
static inline void hb_fibo_lut_build(
        uint16_t          cycle_id,
        const Gpx5File   *gf,
        HbFiboLutEntry   *out)
{
    uint32_t n = gf->hdr.n_tiles;
    for (uint32_t i = 0; i < n; i++) {
        const Gpx5LutEntry *le = &gf->lut[i];

        /* fibo clock position from tile global index */
        uint32_t global_tile = (uint32_t)cycle_id * HB_MAX_TILES + i;
        uint8_t  tick   = fsw_tile_to_phase(global_tile);
        uint8_t  shell  = fsw_tick_to_shell(tick);
        uint8_t  spoke  = fsw_tick_to_spoke(tick);

        /* codec: derive from pipe dispatch using hilbert position
         * pipe rem = global_tile % 3 → pipe entry → codec */
        uint8_t codec = gf->pipes[global_tile % 3u].codec;

        out[i].tile_id    = le->tile_id;
        out[i].cycle_id   = cycle_id;
        out[i].tick       = tick;
        out[i].shell_n    = shell;
        out[i].spoke      = spoke;
        out[i].codec_id   = codec;
        out[i].invert_off = le->invert_offset;
    }
}

/*
 * hb_fibo_lut_seek — O(1) lookup by tick + cycle
 *
 * lut     : array of n entries
 * n       : entry count
 * tick    : fibo clock tick (0..143)
 * cycle   : cycle_id
 * Returns : pointer to first matching entry, NULL if not found
 */
static inline const HbFiboLutEntry *hb_fibo_lut_seek(
        const HbFiboLutEntry *lut,
        uint32_t              n,
        uint8_t               tick,
        uint16_t              cycle)
{
    /* tick = fsw_tile_to_phase(global_tile) != tile_id in general.
     * Linear scan: n<=4096, ~4µs worst case, acceptable for AR seek. */
    for (uint32_t i = 0; i < n; i++) {
        if (lut[i].cycle_id == cycle && lut[i].tick == tick)
            return &lut[i];
    }
    return NULL;
}
