                                    ...
                                    GeoFieldEncodeStats *stats)
{
    ...
    /* Post-encode explicit flush (transaction boundary, not per-chunk) */
    for (uint32_t bi = 0; bi < gf->n_blocks; bi++) {
        FrustumBlock *fb = &gf->blocks[bi];
        uint8_t trit  = (uint8_t)(bi % FGLS_DRAIN_COUNT);
        uint8_t slope = fabric_rotation_state();
        WireResult wr = fabric_wire_commit(fb, trit, slope, POGLS_LAYER_CORE);
        if (wr == WIRE_FENCE_DENY) stats->zone_resets++;
    }
