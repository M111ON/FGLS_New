                                          ...
                                          GeoFieldEncodeStats *stats)
{
    ...
    /* PATH A: heptagon fence write gate (neutral key — store path).
     * FENCE_DENY_LOCKED means slot was locked by a pending reshape;
     * skip write and let reshape drain clear it. */
    FenceResult fr = heptagon_fence_write(blk, shadow_idx,
                                           POGLS_LAYER_CORE,
                                           0u, 0u);   /* rot=0,phase=0 → key=0 neutral */
    if (fr != FENCE_OK) {
        /* Slot gated — trigger reshape to clear it, then retry next encode cycle */
        pogls_atomic_reshape(blk, NULL);
        if (stats) stats->zone_resets++;
        /* Re-check: if reshape cleared the fence, proceed; else skip this chunk */
        fr = heptagon_fence_write(blk, shadow_idx, POGLS_LAYER_CORE, 0u, 0u);
        if (fr != FENCE_OK) goto skel_step;
    }

    /* Data write — fence passed */
    memcpy(blk->data + (size_t)diamond_slot * FGLS_DIAMOND_BYTES,
           chunk, FGLS_DIAMOND_BYTES);

    /* Mark drain active (store path: drain stays closed, just mark occupied).
     * Do NOT call fabric_wire_commit here — that is the explicit drain/flush path. */
    blk->meta.drain_state[drain_idx] |= DRAIN_BIT_ACTIVE;
    /* shadow already marked OCCUPIED by heptagon_fence_write above */

    /* Atomic Reshape check after each write */
