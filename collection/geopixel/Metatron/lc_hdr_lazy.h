...
typedef struct {
    ...
    uint8_t  valid;
    uint8_t  ghosted;
    uint8_t  dirty;    /* 1=seed set, raw not yet built (lazy) */
    uint8_t  _pad[1];
    uint64_t seed_lazy; /* seed stored at write — built on demand  */
    uint32_t wc_lazy;   /* write_count at build time               */
} LCGWChunk;
...
/* ── lcgw_materialize: build raw from seed if dirty (lazy) ──── */
static inline void lcgw_materialize(LCGWChunk *ch)
{
    if (!ch->dirty) return;
    lcgw_build_from_seed(ch->raw, ch->seed_lazy, ch->wc_lazy);
    ch->dirty = 0;
}
                                                 ...
                                                 LCPalette *pal_out)
{
    ...
    if (!ch->valid || ch->ghosted) return NULL;
    lcgw_materialize(ch);   /* lazy: build raw on first read */
    return ch->raw;
}
