                                      ...
                                      uint64_t addr, uint64_t val)
{
    ...
    /* lazy write: store seed only — raw built on demand at read */
    ...
    if (gf->open && ci < gf->chunk_count) {
        ...
        ch->dirty      = 1;   /* flag: needs materialize on read */
        ch->valid      = 1;
        ch->ghosted    = 0;
    }
...
static inline int lcgw_ground_rewind(LCGWGroundStore *gs, uint8_t spoke)
{
    ...
    /* base_wc: write_count when first chunk was built
     * chunk ci was built at write_count = (write_count - chunk_count) + ci
     * Simplest: pass write_count at last build = write_count - 1
     * lcgw_rewind will use base_wc + ci for each chunk */
    uint32_t base_wc = (lane->write_count >= LCGW_GROUND_SLOTS)
                     ? lane->write_count - LCGW_GROUND_SLOTS
                     : 0u;

    int r = lcgw_rewind(lane->gslot, &gs->rb, lane->seeds, base_wc);
    if (r == 0) {
        lane->ghosted = 0;
