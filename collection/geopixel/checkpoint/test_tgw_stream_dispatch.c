...
/* Define guards before any includes to prevent duplicate typedefs */
...
static int test_sd1_spoke_coverage(void) {
    ...
    uint32_t sz = TSTREAM_DATA_BYTES * 720u;   /* full walk coverage */
...
static int test_sd3_stats(void) {
    ...
    static TGWCtx ctx;
    static TGWDispatch d;
    static TGWStreamDispatch sd;
...
static int test_sd5_seam1_proof(void) {
    ...
    static TGWCtx ctx_old;
    static TGWDispatch d_old;
    ...
    static TGWCtx ctx_new;
    static TGWDispatch d_new;
    static TGWStreamDispatch sd_new;
...
/* Minimal stubs for missing globals (defined in real POGLS but not linked) */
#ifdef UNDEFINED_STUBS
// If we hit linking issues, add stubs here
#endif
