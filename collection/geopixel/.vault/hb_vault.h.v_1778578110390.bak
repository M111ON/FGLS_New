        ...
        const char  *out_path)
{
    ...
    uint8_t *zstream  = (uint8_t *)malloc(comp_bound + 16);
    if (!zstream){ free(stream); return HB_ERR_ALLOC; }
    /* header: [0..7] orig_sz64, [8..15] zdata_sz64 (filled after compress) */
    uint64_t orig_sz64 = (uint64_t)stream_sz;
    for (int _i=0;_i<8;_i++) zstream[_i]=(uint8_t)(orig_sz64>>(_i*8));
    uLongf zlen = comp_bound;
    if (compress2(zstream+16, &zlen, stream, (uLong)stream_sz, 9) != Z_OK){
        free(stream); free(zstream); return HB_ERR_ALLOC;
    }
    uint64_t zdata_sz64 = (uint64_t)zlen;
    for (int _i=0;_i<8;_i++) zstream[8+_i]=(uint8_t)(zdata_sz64>>(_i*8));
    free(stream);
    size_t zstream_sz = (size_t)zlen + 16;
    ...
    /* ── Step 3: hamburger encode (multi-cycle aware) ── */
    uint32_t total_tiles = (uint32_t)((W / HBV_TILE_W) * (H / HBV_TILE_H));
    uint32_t n_cycles    = (total_tiles + HB_MAX_TILES - 1u) / HB_MAX_TILES;
    fprintf(stderr,"[hbv_pack] stream_sz=%zu W=%d H=%d tiles=%u cycles=%u\n",
            stream_sz, W, H, total_tiles, n_cycles);
    ...
    /* ── Step 4: write manifest (.gpx5m) if multi-cycle ── */
    if (n_cycles > 1u) {
        char mpath[512];
        snprintf(mpath, sizeof(mpath), "%s.m", out_path);
        FILE *mf = fopen(mpath, "wb");
        if (!mf) return HB_ERR_IO;
        /* manifest: magic(4) + n_cycles(4) + total_tiles(4) + W(4) + H(4) */
        uint8_t mhdr[20];
        memcpy(mhdr, "HBVM", 4);
        _hbv_w4(mhdr+4,  n_cycles);
        _hbv_w4(mhdr+8,  total_tiles);
        _hbv_w4(mhdr+12, (uint32_t)W);
        _hbv_w4(mhdr+16, (uint32_t)H);
        fwrite(mhdr, 1, 20, mf);
        fclose(mf);
        fprintf(stderr,"[hbv_pack] manifest → %s (%u cycles)\n", mpath, n_cycles);
    }
    return HB_OK;
...
{
    /* ── Step 1: check for manifest (multi-cycle) ── */
    uint32_t n_cycles = 1u;
    int      full_W   = HBV_IMAGE_W;
    int      full_H   = 0;
    {
        char mpath[512];
        snprintf(mpath, sizeof(mpath), "%s.m", vault_path);
        FILE *mf = fopen(mpath, "rb");
        if (mf) {
            uint8_t mhdr[20];
            if (fread(mhdr, 1, 20, mf) == 20 && memcmp(mhdr, "HBVM", 4) == 0) {
                n_cycles = _hbv_r4(mhdr+4);
                full_W   = (int)_hbv_r4(mhdr+12);
                full_H   = (int)_hbv_r4(mhdr+16);
            }
            fclose(mf);
            fprintf(stderr,"[hbv_unpack] manifest: cycles=%u W=%d H=%d\n",
                    n_cycles, full_W, full_H);
        }
    }
    /* single-cycle: read img_h from gpx5 header */
    if (n_cycles == 1u) {
        ...
        if (fread(hdr, 1, 24, fh) == 24) {
            uint16_t ty_f = (uint16_t)((hdr[10]<<8)|hdr[11]);
            full_H = (int)ty_f * HBV_TILE_H;
        }
        fclose(fh);
        if (full_H <= 0){ fprintf(stderr,"hbv_unpack: bad gpx5 header\n"); return HB_ERR_IO; }
    }
    fprintf(stderr,"[hbv_unpack] cycles=%u W=%d H=%d\n", n_cycles, full_W, full_H);

    /* ── Step 2: decode all cycles → combined planes ── */
    size_t total_pixels = (size_t)full_W * (size_t)full_H;
    int *all_Y  = (int *)calloc(total_pixels, sizeof(int));
    int *all_Cg = (int *)calloc(total_pixels, sizeof(int));
    int *all_Co = (int *)calloc(total_pixels, sizeof(int));
    if (!all_Y || !all_Cg || !all_Co) {
        free(all_Y); free(all_Cg); free(all_Co); return HB_ERR_ALLOC;
    }
    size_t pixel_off = 0u;
    int r = HB_OK;
    ...
    for (uint32_t cyc = 0; cyc < n_cycles; cyc++) {
        char cyc_path[512];
        if (n_cycles == 1u) {
            snprintf(cyc_path, sizeof(cyc_path), "%s", vault_path);
        } else {
            const char *dot = strrchr(vault_path, '.');
            if (dot) {
                size_t bl = (size_t)(dot - vault_path);
                snprintf(cyc_path, sizeof(cyc_path), "%.*s_c%03u%s",
                         (int)bl, vault_path, cyc, dot);
            } else {
                snprintf(cyc_path, sizeof(cyc_path), "%s_c%03u", vault_path, cyc);
            }
        }
        int cyc_h = 0;
        {
            FILE *fh = fopen(cyc_path, "rb");
            if (!fh){ fprintf(stderr,"hbv_unpack: cannot open %s\n", cyc_path);
                      free(all_Y); free(all_Cg); free(all_Co); return HB_ERR_IO; }
            uint8_t hdr[24];
            if (fread(hdr, 1, 24, fh) == 24) {
                uint16_t ty_f = (uint16_t)((hdr[10]<<8)|hdr[11]);
                cyc_h = (int)ty_f * HBV_TILE_H;
            }
            fclose(fh);
        }
        if (cyc_h <= 0){ free(all_Y); free(all_Cg); free(all_Co); return HB_ERR_IO; }

        HbImageOut dec;
        memset(&dec, 0, sizeof(dec));
        r = hamburger_decode_image(cyc_path, &dec, full_W, cyc_h, HBV_TILE_W, HBV_TILE_H);
        if (r != HB_OK) {
            fprintf(stderr,"hbv_unpack: cycle %u decode failed (%d)\n", cyc, r);
            hb_image_out_free(&dec); free(all_Y); free(all_Cg); free(all_Co); return r;
        }
        size_t cyc_px = (size_t)full_W * (size_t)cyc_h;
        memcpy(all_Y  + pixel_off, dec.Y,  cyc_px * sizeof(int));
        memcpy(all_Cg + pixel_off, dec.Cg, cyc_px * sizeof(int));
        memcpy(all_Co + pixel_off, dec.Co, cyc_px * sizeof(int));
        pixel_off += cyc_px;
        hb_image_out_free(&dec);
        fprintf(stderr,"[hbv_unpack] cycle %u/%u cyc_h=%d\n", cyc, n_cycles-1u, cyc_h);
    }

    HbImageOut combined;
    combined.Y=all_Y; combined.Cg=all_Cg; combined.Co=all_Co;
    combined.w=full_W; combined.h=full_H;
    combined.tw=HBV_TILE_W; combined.th=HBV_TILE_H;

    /* ── Step 3: planes → zstream ── */
    size_t zstream_sz = 0;
    uint8_t *zstream  = _hbv_planes_to_stream(&combined, &zstream_sz);
    free(all_Y); free(all_Cg); free(all_Co);
    if (!zstream) return HB_ERR_ALLOC;

    /* ── Step 4: zlib-decompress ── */
    if (zstream_sz < 16){ free(zstream); return HB_ERR_TILES; }
    uint64_t orig_sz64 = 0;
    for (int _i=0;_i<8;_i++) orig_sz64 |= ((uint64_t)zstream[_i])<<(_i*8);
    uint64_t zdata_sz64 = 0;
    for (int _i=0;_i<8;_i++) zdata_sz64 |= ((uint64_t)zstream[8+_i])<<(_i*8);
    ...
    if (uncompress(stream, &stream_sz, zstream+16, (uLong)zdata_sz64) != Z_OK){
