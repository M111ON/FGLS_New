...
Metal detector pattern:
  ...
  PASS    → core tensor ผ่าน geometry gate
...
# ── CROSS self-inverse LUT (frozen invariant) ────────────
# cpair(cpair(x)) = x  ←  verified
...
class GeoCodebook(nn.Module):
    ...
    def traverse(self, idx, mode=0):
        """
        ORBITAL  +1        (local step)
        CHIRAL   +n//2     (mirror zone, half-cycle)
        CROSS    zone ↔ partner  — SELF-INVERSE: cpair(cpair(x))=x ✓
        HUB      → zone anchor  (collapse)
        """
        if mode == 0:
            return (idx + 1) % self.n_codes

        if mode == 1:
            return (idx + self.n_codes // 2) % self.n_codes

        if mode == 2:
            # CROSS fix: work in walk-space, apply CROSS_LUT, invert back
            # enc = (idx * 37) % 720  →  zone  →  partner_zone  →  new_enc  →  new_idx
            # self-inverse because CROSS_LUT[CROSS_LUT[z]] = z (verified below)
            encs         = (idx * 37) % 720
            zones        = encs // 60
            offset_in_zone = encs % 60
            partner_zones  = CROSS_LUT.to(idx.device)[zones]
            new_encs       = partner_zones * 60 + offset_in_zone
            # invert walk: enc = (idx*37)%720 → idx = (enc * inverse_37) % n_codes
            # 37^-1 mod 720 = 253  (37*253 = 9361 = 13*720 + 1)
            INV37 = 253
            new_idx = (new_encs * INV37) % self.n_codes
            return new_idx

        if mode == 3:
            encs  = (idx * 37) % 720
            zones = encs // 60
            return (zones * (self.n_codes // N_ZONES)) % self.n_codes
...
class BermudaGate(nn.Module):
    ...
    def decode(self, idx):
        z_q = self.codebook.codes[idx % self.codebook.n_codes]
        geo = GEO_TABLE[idx % N_CODES]
        return self.decoder(torch.cat([z_q, geo], dim=-1))

    def forward(self, x_flat, mode=0):
...
def bermuda_pass(gate: BermudaGate,
    ...
    gate.eval()
    with torch.no_grad():
        core_out, idx_in, idx_out, _ = gate(core, mode)
    ...
    x_out      = x_out_flat.reshape(out_shape + (D,))
...
def train_gate(dim=128, n_codes=60, epochs=600):
    ...
    for ep in range(epochs):
        ...
        # diversity loss: push encoder z apart
        with torch.no_grad():
            z = gate.encoder(core)
...
def verify_cross_lut():
    ...
    print("CROSS_LUT self-inverse: PASS ✓")
...
def run():
    ...
    with torch.no_grad():
        ...
        for mode, name in enumerate(["ORBITAL","CHIRAL","CROSS","HUB"]):
            ...
            zones_in  = ((snap.idx_in  * 37) % 720 // 60).unique().tolist()
            zones_out = ((gate.codebook.traverse(snap.idx_in, mode) * 37) % 720 // 60).unique().tolist()
