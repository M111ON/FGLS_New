...
/*
 * cardioid_pass(pos, r_data) — 2-gate hybrid
 *
 * Gate 1 — Geometric floor (deterministic):
 *   r_geo_q8 = A × (256 + cos[pos]) / 256
 *   r_geo_q8 < GEO_MIN_Q8 → CUSP block (anomaly, noise rejection)
 *
 * Gate 2 — Data gate (adaptive):
 *   r_data_scaled ≤ r_geo → EXPRESS, else LINEAR
 *
 * Returns: CARDIOID_PASS_CUSP(-1) | CARDIOID_PASS_LINEAR(0) | CARDIOID_PASS_EXPRESS(1)
 *
 * Expected bypass: ~60-65% (vs ~74% single-gate)
 * Cusp zone (~90 slots where cos < -0.75): always CARDIOID_PASS_CUSP
 */
#define CARDIOID_PASS_CUSP    (-1)
#define CARDIOID_PASS_LINEAR    0
#define CARDIOID_PASS_EXPRESS   1

/*
 * GEO_MIN_Q8: geometric floor threshold (Q8 scale, range 0..512)
 *   80 ≈ r=0.31 → good balance, ~60-65% express
 *   Tune: 64 (0.25, more bypass) .. 90 (0.35, less bypass)
 */
#define CARDIOID_GEO_MIN_Q8  140

static inline int cardioid_pass(uint16_t pos, uint16_t r_data)
{
    int32_t cos_val  = _cardioid_cos_lut[pos % CARDIOID_TRING_LEN];
    int32_t r_geo    = (int32_t)CARDIOID_A *
                       ((int32_t)CARDIOID_SCALE + cos_val);
    int32_t r_geo_q8 = r_geo >> 8;

    /* Gate 1: geometric floor — cusp/low-geo hard block */
    if (r_geo_q8 < (int32_t)CARDIOID_GEO_MIN_Q8)
        return CARDIOID_PASS_CUSP;

    /* Gate 2: data gate — adaptive signal check */
    return (((int32_t)r_data << 8) <= r_geo)
           ? CARDIOID_PASS_EXPRESS
           : CARDIOID_PASS_LINEAR;
}
...
tgw_cardioid_decide(uint64_t enc, uint32_t value, uint16_t m)
{
    ...
    int pass      = cardioid_pass(d.pos, d.r);
    d.use_express = (pass == CARDIOID_PASS_EXPRESS) ? 1 : 0;
