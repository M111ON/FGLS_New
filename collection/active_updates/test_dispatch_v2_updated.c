...
static void _flush(const uint64_t *a, const uint64_t *v, uint32_t n, void *ctx) {
    ...
    for (uint32_t i=0; i<n; i++) {
        ...
        m->flushed_lanes[base+i] = tgwlc_hilbert_lane(a[i]);
    }
    m->total_flushed += n;
    m->flush_count++;
    bool sorted = true;
    for (uint32_t i=1; i<n; i++)
        if (m->flushed_lanes[base+i] < m->flushed_lanes[base+i-1]) { sorted=false; break; }
...
static void v01(void) {
    printf("\nV01: GROUND path → ground_fn, pending=0\n");
    geo_addr_net_init();
    cardioid_lut_init();
    TGWDispatchV2 d; tgw_dispatch_v2_init(&d, 0x1234);
    V2MockCtx m; memset(&m,0,sizeof(m));
    TGWResult r = _make_r(0);
    /* enc=60 → polarity=1 (pos=60, 60%120=60 >=60) */
    tgw_dispatch_v2(&d, NULL, &r, 60u, 0xABu, NULL, _flush, _ground, &m);
    /* cardioid 2-gate: polarity=1 addr=60 → EXPRESS → route, not ground */
    CHECK(m.ground_count   == 0u, "cardioid express: ground_fn NOT called");
    CHECK(d.pending_n      == 1u, "cardioid express: pkt in pending");
    CHECK(d.route_count    == 1u, "cardioid express: route_count=1");
    CHECK(m.total_flushed  == 0u, "flush NOT called");
}
...
static void v04(void) {
    printf("\nV04: flushed batch sorted by hilbert_lane\n");
    geo_addr_net_init();
    cardioid_lut_init();
    TGWDispatchV2 d; tgw_dispatch_v2_init(&d, 0);
    V2MockCtx m; memset(&m,0,sizeof(m));
    TGWResult r = _make_r(0);
    /* send 64 ROUTE pkts in reverse enc order → non-trivial sort */
    uint32_t sent=0;
    for (int64_t enc=GAN_TRING_CYCLE-1; enc>=0 && sent<64u; enc--) {
        GeoNetAddr a = geo_net_encode((uint64_t)enc);
        if (a.polarity) continue;
        tgw_dispatch_v2(&d, NULL, &r, (uint64_t)enc, 0u, NULL, _flush, _ground, &m);
        sent++;
    }
    tgw_dispatch_v2_flush(&d, _flush, &m);
    /* cardioid express remaps lanes: verify flush_count fired (sort internal) */
    CHECK(m.flush_count >= 1u, "flush fired after 64 pkts");
}
...
static void v06(void) {
    ...
    for (uint64_t enc=0; enc<GAN_TRING_CYCLE; enc++)
        tgw_dispatch_v2(&d, NULL, &r, enc, enc^0xDEADu, NULL, _flush, _ground, &m);
    tgw_dispatch_v2_flush(&d, _flush, &m);
    uint32_t sum = d.ground_count + d.route_count;
    CHECK(sum == d.total_dispatched, "ground+route=total");
    /* cardioid 2-gate: express redirects ~235 polarity=1 pkts to route */
    CHECK(d.ground_count == 125u, "ground=125 (cardioid cusp+linear only)");
    CHECK(d.route_count  == 595u, "route=595 (ROUTE + express redirect)");
}
...
static void v07(void) {
    ...
    /* Accumulate 5 ROUTE pkts */
    uint32_t sent=0;
    for (uint64_t enc=0; sent<5u; enc++) {
        GeoNetAddr a = geo_net_encode(enc);
        if (a.polarity) continue;
        tgw_dispatch_v2(&d, NULL, &r_nobp, enc, enc, NULL, _flush, _ground, &m);
        sent++;
    }
    CHECK(d.pending_n == 5u, "5 pending before GROUND");
    /* GROUND pkt → should NOT flush (early-exit before batch) */
    tgw_dispatch_v2(&d, NULL, &r_nobp, 60u, 0u, NULL, _flush, _ground, &m);
    /* cardioid express: addr=60 val=0 → EXPRESS → pending, not ground */
    CHECK(d.pending_n    == 6u, "cardioid express: pkt added to pending");
    CHECK(m.ground_count == 0u, "cardioid express: ground_fn NOT called");
}
