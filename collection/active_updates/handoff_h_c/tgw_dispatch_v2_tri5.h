...
#include "tgw_tri5_wire.h"         /* tri[5] FLOW drop-in hook               */
...
#include "core/pogls_engine/lc_twin_gate.h"
...
typedef struct {
    ...
    uint32_t        tri5_count;         /* tri[5] Metatron override fires  */
                                     ...
                                     void            *fn_ctx)
{
    ...
    if (geo.polarity) {
        ...
        /* TRI5: geometry override — check before cardioid */
        TRI5_HOOK_A(d, addr, val, geo, flush_fn, fn_ctx);
    ...
    if (verdict) {
        /* ROUTE: push — HOOK B tri5 + cardioid override */
        CardioidDecision cd = tgw_cardioid_decide(addr, (uint32_t)val,
                                                   CARDIOID_M_DEFAULT);
        uint16_t bucket;
        TRI5_HOOK_B(bucket, addr, geo, cd);
        if (cd.use_express) d->express_count++;
        if (!cd.use_express && cardioid_pass(cd.pos, cd.r) == CARDIOID_PASS_CUSP)
            d->cusp_block_count++;
        _v2_push(d, addr, val, bucket, flush_fn, fn_ctx);
