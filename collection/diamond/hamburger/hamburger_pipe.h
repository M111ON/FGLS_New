...
static inline void hb_invert_record_enc(
        ...
        uint8_t codec_id)    /* codec used — reserved for future LUT use */
{
    ...
    /* check all-zero: clear = perfect codec prediction
     * RAW/NONE codecs always produce output == input, which is NOT
     * "perfect prediction" — the decoder has no way to reconstruct
     * without storing the residual. Only skip storage for predictive
     * codecs (SEED, DELTA, etc.) where output truly matches input. */
    int clear = (orig_sz == enc_sz) &&
                (codec_id != GPX5_CODEC_RAW &&
                 codec_id != GPX5_CODEC_NONE);
    if (clear) {
