...
#define L2_MAXBYTES       (1 + L2_TILES * HEX_TILE_MAXBYTES)
...
static uint8_t _xor_pred(const uint8_t *dec, int scan_pos) {
    if (scan_pos == 0) return 128;
    int ring_pos = scan_pos - 1;
    uint8_t center = dec[6];
    uint8_t cyl;
    if (ring_pos == 0) {
        cyl = center;
    } else {
    ...
    if      (diff == 0)    return cyl;
    else if (diff < 0x10)  return (cyl + star) >> 1;
    else if (diff < 0x80)  return (cyl + star) >> 1;
    else                   return (~star) & 0xFF;
}
