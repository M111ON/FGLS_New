...
/* ═══════════════════════════════════════════════════════════════════════
   Rotation 2‑Axis Canonicalization
   ═══════════════════════════════════════════════════════════════════════
   rotA: (route×5 + set×6 + phase×3) % 360
         pentagon × hexagon × time — topology space

   rotB: (face×15 + edge×9 + z×7) % 360
         dodecahedron coordinate basis — position space

   flow_fp: seed ^ (rotA<<32) ^ (rotB<<16)
         used ONLY for verify / refine — NOT for grouping
════════════════════════════════════════════════════════════════════════ */

/* Identity axis (topology): route, set, phase */
static inline uint16_t fibo_rotA(uint8_t route, uint8_t set, uint16_t phase) {
    return (uint16_t)(((uint32_t)route * 5u + (uint32_t)set * 6u + (uint32_t)phase * 3u) % 360u);
}

/* Orientation axis (position): face, edge, z */
static inline uint16_t fibo_rotB(uint8_t face, uint8_t edge, uint16_t z) {
    return (uint16_t)(((uint32_t)face * 15u + (uint32_t)edge * 9u + (uint32_t)z * 7u) % 360u);
}

/* Combined fingerprint — verify/refine only */
static inline uint64_t fibo_flow_fp(uint64_t seed,
                                     uint8_t route, uint8_t set,
                                     uint16_t phase,
                                     uint8_t face, uint8_t edge, uint16_t z)
{
    uint64_t a = fibo_rotA(route, set, (uint8_t)(phase & 0xFFu));
    uint64_t b = fibo_rotB(face, edge, z);
    return seed ^ (a << 32) ^ (b << 16);
}

/* Coarse group key — grouping only (NO rotation) */
static inline uint64_t fibo_group_key(uint64_t seed, uint8_t set) {
    return seed ^ ((uint64_t)set << 8);
}
