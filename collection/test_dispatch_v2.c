...
typedef struct { LCTwinGateCtx_placeholder *_; uint64_t *_bundle; } TGWCtx;
...
/* ── mock callbacks ───────────────────────────────────────── */
typedef struct {
    uint64_t flushed_addrs[TGW_V2_BATCH_MAX * 8];
    uint64_t flushed_vals [TGW_V2_BATCH_MAX * 8];
    uint16_t flushed_lanes[TGW_V2_BATCH_MAX * 8];
    uint32_t flush_count;
    uint32_t total_flushed;
    uint64_t ground_addrs[TGW_V2_BATCH_MAX * 8];
...
static void v01(void) {
    ...
    CHECK(m.ground_count   == 1u, "ground_fn called");
    CHECK(d.pending_n      == 0u, "pending empty");
...
static void v04(void) {
    ...
    CHECK(m.last_flush_sorted, "flushed batch sorted ASC by hilbert_lane");
}
...
static void v07(void) {
    ...
    CHECK(m.ground_count == 1u, "ground_fn called for GROUND pkt");
}
