...
if __name__ == "__main__":
    ...
    for m in range(4):
        ...
        steps = [r[2].scale for r in group]
        if len(xdiffs) >= 2:
            xratio = [f"{d/max(xdiffs[0],1):.3f}" for d in xdiffs]
            cratio = [f"{d/max(cdiffs[0],1):.3f}" for d in cdiffs]
            print(f"  {mname:10s}  steps={steps}")
            print(f"  {'':10s}    out_xdiff = {xdiffs}  ratios={xratio}")
            print(f"  {'':10s}    code_xdiff = {cdiffs}  ratios={cratio}")
            # Check ramp > 2× for last step vs first
            if len(xdiffs) >= 4:
                max_ratio = xdiffs[-1] / max(xdiffs[0], 1)
                tag = "✓" if max_ratio > 2.0 else "✗"
                ramp_ok = ramp_ok and (max_ratio > 2.0)
                print(f"  {'':10s}    max_ratio={max_ratio:.2f}x  {tag}")

    print(f"\n{'─'*40}")
    print(f"RAMP > 2×: {'ALL PASS ✓' if ramp_ok else 'BELOW TARGET — tune diversity loss weight or increase epochs'}")
    print(f"Next: diversity loss weight=0.3 in train_gate(), ramp threshold = div_margin + div_code + entropy")
