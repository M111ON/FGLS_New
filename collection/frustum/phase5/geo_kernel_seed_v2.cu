...
uint32_t _d_coset_checksum(uint64_t seed, uint8_t coset)
{
    ...
    uint64_t acc       = cseed;
    uint32_t coset_chk = (uint32_t)(cseed ^ (cseed >> 32));  /* seed anchor */
                             ...
                             ApexResult * __restrict__ results)
{
    ...
    if (tid == 0u) {
        ...
        /* verify: XOR fold must equal master (always true if consistent) */
        uint32_t recheck = 0u;
        for (uint8_t c = 0; c < K_COSET_COUNT; c++) recheck ^= s_chk[c];
...
extern "C"
#endif
FGLS_EXPORT cudaError_t geo_seed_launch(const ApexSeed *d_seeds,
