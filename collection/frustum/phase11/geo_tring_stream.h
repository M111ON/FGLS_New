...
#define TSTREAM_SEG_PKT_BYTES  4112u  /* enc(4)+size(2)+crc16(2)+seg_id(4)+seg_total(4)+data(4096) */
#define TSTREAM_MAX_SEGS       0xFFFFFFFFu /* uint32 — supports files up to ~11.5 PB */

/* max file size = 2^32 segments × 720 chunks × 4096B = ~11.5 PB */

typedef struct {
    uint32_t enc;                      /* GEO_WALK[chunk_id]              */
    uint16_t size;                     /* actual data bytes                */
    uint16_t crc16;                    /* CRC16 over data[0..size-1]       */
    uint32_t seg_id;                   /* which segment (0-based)          */
    uint32_t seg_total;                /* total segments for this file     */
    uint8_t  data[TSTREAM_DATA_BYTES];
} TStreamPktSeg;
 ...
 * O(1)                                                            */
static inline uint32_t tstream_seg_count(uint64_t fsize)
{
    if (fsize == 0u) return 0u;
    uint64_t chunks = (fsize + TSTREAM_DATA_BYTES - 1u) / TSTREAM_DATA_BYTES;
    return (uint32_t)((chunks + TSTREAM_MAX_PKTS - 1u) / TSTREAM_MAX_PKTS);
}
...
static inline uint16_t tstream_slice_seg(TStreamPktSeg    out_pkts[TSTREAM_MAX_PKTS],
                                          ...
                                          uint32_t         seg_total)
{
    uint64_t seg_off  = (uint64_t)seg_id * TSTREAM_MAX_PKTS * TSTREAM_DATA_BYTES;
    if (seg_off >= fsize) return 0u;

    uint64_t seg_size = fsize - seg_off;
...
static inline uint64_t tstream_reconstruct_segmented(
...
{
    uint64_t written = 0u;
    for (uint32_t s = 0u; s < seg_total; s++) {
