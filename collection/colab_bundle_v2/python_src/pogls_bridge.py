...
def _load_lib(path: Path) -> ctypes.CDLL:
    ...
    # fibo_addr_batch (bulk)
    lib.poglsex_fibo_addr_batch.restype  = None
    lib.poglsex_fibo_addr_batch.argtypes = [
...
class PoglsBridge:
    ...
    def fibo_addr_batch(self, seeds):
        seeds_arr = (ctypes.c_uint64 * len(seeds))(*seeds)
        results_arr = (ctypes.c_uint64 * len(seeds))()
        self._lib.poglsex_fibo_addr_batch(seeds_arr, results_arr, len(seeds))
