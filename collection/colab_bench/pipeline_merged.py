...
Pipeline:
  ...
  GEO_SEEK  → tw_capture + frame_seek permutation (NEW)
...
from geo_frame_seek import (
    frame_seek, frame_enc, tw_capture_int_combined,
    geo_frame_seek_verify, FRAME_EDGES, FRAME_CYCLE,
    FRAME_FACE_SZ, FRAME_ICO_NODES, TWCaptureInt,
)
...
class PipelineSnapshot:
    ...
    geo_seek:         dict   # geo_frame_seek stats (n_frames, delta_ratio, etc.)
...
def run_pipeline(gate: BermudaGate,
    ...
    # ── 2b. GEO SEEK — tw_capture + frame_seek permutation ────
    # Convert core bytes to geometric captures, permute by frame order
    geo_seek_stats = None
    try:
        core_bytes = core.cpu().numpy().tobytes()
        n_pairs = len(core_bytes) // 2
        n_frames = min(n_pairs // FRAME_EDGES, 1000)
        if n_frames > 0:
            captures = []
            scale = 207360 // 128  # TW_SCALE // 128
            for i in range(min(n_pairs, n_frames * FRAME_EDGES)):
                b0 = core_bytes[2*i]
                b1 = core_bytes[2*i + 1]
            ...
            # Permute by frame_seek ordering
            permuted = []
            for fi in range(n_frames):
                ...
                for ci in range(FRAME_EDGES):
                    ...
                    if src_idx < len(captures):
                        permuted.append(captures[src_idx])

            # Delta encode: frame 0 = seed, frame i>0 = delta from seed
            same_count = 0
            if n_frames > 1:
            ...
            geo_seek_stats = {
                ...
                'delta_ratio': same_count / max(1, (n_frames - 1) * FRAME_EDGES),
            }
    except Exception as e:
        geo_seek_stats = {'error': str(e)}
    ...
    snap = PipelineSnapshot(
        ...
        geo_seek=geo_seek_stats or {'n_frames': 0, 'delta_ratio': 0})
