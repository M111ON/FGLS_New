...
#include "tgw_cardioid_express.h"  /* Cardioid × TRing express lane         */
                                     ...
                                     void            *fn_ctx)
{
    ...
    if (geo.polarity) {
        /* HOOK A: cardioid express bypass before GROUND sink */
    ...
    if (!r->gpr.blueprint_ready) {
        d->no_blueprint++;
        d->route_count++;   /* no_blueprint is a ROUTE-path subset */
        /* HOOK B: cardioid override hilbert_idx for express packets
         * next_pos is 0..719 — map to hilbert range 0..119 via /6 */
        CardioidDecision cd = tgw_cardioid_decide(addr, (uint32_t)val,
                                                   CARDIOID_M_DEFAULT);
        uint16_t bucket = cd.use_express
                          ? (uint16_t)(cd.next_pos / 6u)   /* 720/6=120 buckets */
                          : geo.hilbert_idx;
    ...
    if (verdict) {
        /* ROUTE: push Hilbert-keyed slot */
        _v2_push(d, addr, val, geo.hilbert_idx, flush_fn, fn_ctx);
