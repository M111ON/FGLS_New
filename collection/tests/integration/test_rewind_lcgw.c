...
static void r01(void) {
    ...
    uint8_t  ci       = (lane->chunk_cursor == 0) ? (uint8_t)(LCGW_GROUND_SLOTS - 1u) : (uint8_t)(lane->chunk_cursor - 1u);
    INFO("spoke=%u  seed[%u]=0x%016" PRIx64 "  expected=0x%016" PRIx64 "\n",
         spoke, ci, (uint64_t)lane->seeds[ci], (uint64_t)expected);
...
static void r02(void) {
    ...
    INFO("spoke=%u  ci=%u  seed=0x%016" PRIx64 "  wc=%u  raw_match=%d\n",
         spoke, ci, (uint64_t)seed, wc, match);
...
static void r04(void) {
    ...
    uint8_t ci_last = (lane->chunk_cursor == 0) ? (uint8_t)(LCGW_GROUND_SLOTS - 1u)
                                                 : (uint8_t)(lane->chunk_cursor - 1u);
    ...
    INFO("spoke=%u  seed=0x%016" PRIx64 "  wc=%u  reconstruct_match=%d\n",
         spoke, (uint64_t)seed, wc, match);
...
static void r05(void) {
    ...
    uint32_t ghost_before = gs.total_ghosted;
    LCDeleteResult dr = lcgw_ground_delete(&gs, spoke);
    INFO("delete.status=%d  chunks_ghosted=%u  total_ghosted=%u\n",
         dr.status, dr.chunks_ghosted, gs.total_ghosted);

    CHECK(dr.status == 0, "delete status OK");
    CHECK(dr.chunks_ghosted > 0u, "delete ghosted one or more chunks");
    CHECK(gs.total_ghosted == ghost_before + 1u, "ground total_ghosted incremented");
}
    ...
    for (uint32_t i = 0; i < n; i++) {
        for (uint32_t j = i + 1; j < n; j++) {
            if (lane->seeds[i] == lane->seeds[j]) {
                distinct = 0;
                INFO("seeds[%u]==seeds[%u]=0x%016" PRIx64 "  COLLISION\n",
                     i, j, (uint64_t)lane->seeds[i]);
            }
        }
    }
...
static void r07(void) {
    ...
    for (uint32_t sp = 0; sp < 6; sp++) {
        LCGWSpokeLane *lane = &gs.lanes[sp];
        uint8_t ci = (lane->chunk_cursor == 0) ? (uint8_t)(LCGW_GROUND_SLOTS - 1u)
                                               : (uint8_t)(lane->chunk_cursor - 1u);
        uint64_t seed = lane->seeds[ci];
        uint32_t wc   = lane->write_count - 1u;
        ...
        if (!ok) all_ok = 0;
        INFO("spoke[%u] reconstruct=%s  seed=0x%016" PRIx64 "\n",
             sp, ok ? "OK" : "FAIL", (uint64_t)seed);
    }
...
static void r08(void) {
    ...
    INFO("seed=0x%016" PRIx64 "  build(wc=0)!=build(wc=1): %s\n",
         (uint64_t)seed, raws_differ ? "YES" : "NO");
