...
#include "geo_tring_walk.h"      /* SEAM 3: tile index -> TRing walk enc */
#include "geo_o4_connector.h"    /* O5: O4 grid bridge                   */
#include "gpx4_container.h"      /* GPX4 multi-layer container           */
#include "geo_gpx_anim.h"        /* GPX4 animated pattern support        */
...
#define DEDUP_TABLE_SIZE  4096
     ...
     * ──────────────────────────────────────────────────────────────────── */
    {
        ...
        for(int i=0; i<NT && gpx_rgb; i++){
            ...
            O4GridCtx ctx;
            uint8_t tx = (uint8_t)(i % TW);   /* pixel coords: raster */
            uint8_t ty = (uint8_t)(i / TW);
            uint16_t tile_enc = tring_walk_enc((uint32_t)i); /* SEAM 3: dispatch enc */
            (void)tile_enc; /* routed via tgw_dispatch_v2 in integration layer */
...
static int decode_gpx4(const char *gpx_path, const char *out_bmp){
    ...
    for(int i=0; i<NT; i++){
        ...
        uint16_t tile_enc = tring_walk_enc((uint32_t)i); /* SEAM 3 */
        (void)tile_enc;
        int x0 = tx*TILE, x1 = x0+TILE; if(x1>W) x1=W;
        int y0 = ty*TILE, y1 = y0+TILE; if(y1>H) y1=H;
