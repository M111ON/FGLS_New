...
static inline TwinBridgeStats twin_bridge_stats(const TwinBridge *b)
{
    ...
    s.write_density = b->total_ops > 0
                    ...
                    : 0.0f;
    s.lc_warp      = b->lc_gate.gate_counts[LC_GATE_WARP];
    s.lc_route     = b->lc_gate.gate_counts[LC_GATE_ROUTE];
    s.lc_ground    = b->lc_gate.gate_counts[LC_GATE_GROUND];
    s.lc_collision = b->lc_gate.gate_counts[LC_GATE_COLLISION];
    return s;
}
