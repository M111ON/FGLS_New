...
typedef struct {
    void    *base_addr;     /* base address of the region */
    void    *alloc_base;    /* allocation base (Win: AllocationBase, Linux: same as base) */
    size_t   region_size;   /* size of the region in bytes */
    uint32_t state;         /* POGLS_MEM_* */
    uint32_t protect;       /* POGLS_PAGE_* */
} PoglsMemInfo;

/* Query memory region containing addr.
 * Returns 0 on success, -1 if addr is invalid/unmapped.
 *
 * Windows: wraps VirtualQuery + MEMORY_BASIC_INFORMATION
 * Linux:   parses /proc/self/maps on each call (maps are typically < 1K lines)
 */
...
/* Check if addr points to committed readable/writable memory.
 * Convenience wrapper around pogls_query_memory. */
...
/* ══════════════════════════════════════════════════════════════════
 * Module Path — get current executable directory
 * ══════════════════════════════════════════════════════════════════ */

/* Fill buf with the directory of the current executable (with trailing separator).
 * Returns 0 on success, -1 on error.
 *
 * Windows: GetModuleFileNameA + strrchr '\\'
 * Linux:   readlink /proc/self/exe + strrchr '/'
 */
