...
static uint32_t ds_encode(uint8_t *out, const uint8_t block[64]) {
    ...
    /* ── GeoField Tier 1: Hilbert+Peano maze classification ──
     * Reinterpret 64B chunk as 4×4×4 cube (x=col, y=row, z=floor).
     * NOTE: geo_jump default is 4×4×3 (48B/tower), but _hilbert_idx
     * and _peano_idx accept any grid size — 4×4×4 works fine.
     * Hilbert order vs Peano order → two different linearizations.
     * XOR both arrangements → matching bytes = geometric invariants.
     * High match count → strong geometric structure → SUB-friendly. */
    ...
    int _use_sub = (_hp_matches >= 32);  /* ≥ half match → geometric */
