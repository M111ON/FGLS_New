"""
test_roundtrip_b.py — Phase A4+B1: Adaptive chunk + LetterCube roundtrip

Flow: file → flow_chunk → LetterCube per segment → encode → decode → verify
"""
import sys, os, time, ctypes, hashlib, struct

sys.path.insert(0, os.path.dirname(__file__))
import geofield_ctypes as gf

CHUNK_SZ = 64
GPXL_MAGIC = b'GPXL'
GPXL_VERSION = 2
GPXL_HEADER_SZ = 48
LC_BUF = 76  # LetterCube serialized size


def make_lettercube_for_segment(seg_idx, seg_len):
    """Create a LetterCube for a segment based on its position."""
    buf = (ctypes.c_uint8 * LC_BUF)()
    gf._lib.geofield_lc_init(buf)

    # Assign 6 lanes based on segment geometry
    # Complementary pairs: 0<->12, 1<->13, 2<->14
    for lane, pair, angle in [(0, 0, 0), (1, 12, 0),
                               (2, 1, 1), (3, 13, 1),
                               (4, 2, 2), (5, 14, 2)]:
        gf._lib.geofield_lc_assign(buf, lane, pair, angle)

    # Bond complementary pairs
    gf._lib.geofield_lc_bond(buf, 0, 1)
    gf._lib.geofield_lc_bond(buf, 2, 3)
    gf._lib.geofield_lc_bond(buf, 4, 5)

    # Assemble (lock all pending)
    gf._lib.geofield_lc_assemble(buf)

    return bytes(buf)


def roundtrip_encode(data):
    """Encode data with adaptive chunks + LetterCube headers."""
    original_size = len(data)
    xxh64_val = gf.xxh64(data)

    # Step 1: Adaptive chunking
    max_segs = (original_size // 32) + 256
    offsets = (ctypes.c_uint64 * max_segs)()
    lengths = (ctypes.c_uint64 * max_segs)()
    n_segments = gf._lib.geofield_flow_chunk(
        data, original_size, 32, 4096, offsets, lengths, max_segs
    )
    if n_segments <= 0:
        raise ValueError(f"flow_chunk returned {n_segments}")

    stats = {
        'n_segments': n_segments,
        'original_size': original_size,
        'xxh64': xxh64_val,
        'skel_hits': [0] * 6,
        'shell_hits': [0] * 3,
    }

    # Step 2: Process each segment — Diamond Shell + Skeleton
    segments = []
    total_blocks = 0

    for si in range(n_segments):
        seg_off = offsets[si]
        seg_len = lengths[si]
        seg_data = bytes(data[seg_off:seg_off + seg_len])

        block_strategies = []
        for bi in range(0, seg_len, CHUNK_SZ):
            block = seg_data[bi:bi + CHUNK_SZ]
            if len(block) < CHUNK_SZ:
                block = block + b'\x00' * (CHUNK_SZ - len(block))

            shell_result = gf.diamond_classify(block)
            prev_block = bytes(segments[-1][-1][-1]) if segments and segments[-1][-1] else None
            skel_result = gf.skel_decide(block, prev_block, prev_block is not None)

            stats['skel_hits'][skel_result.strategy] += 1
            stats['shell_hits'][shell_result.flag] += 1
            block_strategies.append((skel_result.strategy, shell_result.flag))
            total_blocks += 1

        segments.append((seg_data, block_strategies))

    stats['total_blocks'] = total_blocks

    # Step 3: Generate LetterCube per segment
    lc_states = []
    for si in range(n_segments):
        seg_len = lengths[si]
        lc_buf = make_lettercube_for_segment(si, seg_len)
        lc_states.append(lc_buf)

    # Step 4: Serialize
    coord_section_sz = 20 * n_segments
    lc_section_sz = LC_BUF * n_segments

    buf = bytearray()

    # Header
    buf += GPXL_MAGIC
    buf += struct.pack('<H', GPXL_VERSION)
    buf += struct.pack('<B', 0)  # gp_level
    buf += struct.pack('<B', 0)  # base
    buf += struct.pack('<H', 0)  # side
    buf += struct.pack('<I', n_segments)
    buf += struct.pack('<Q', original_size)
    buf += struct.pack('<Q', xxh64_val)
    buf += struct.pack('<I', coord_section_sz + lc_section_sz)
    buf += b'\x00' * (GPXL_HEADER_SZ - len(buf))

    # Coord records
    for si in range(n_segments):
        seg_data, block_strats = segments[si]
        first_strategy = block_strats[0][0] if block_strats else 5
        first_block = bytes(seg_data[:CHUNK_SZ]) if len(seg_data) >= CHUNK_SZ else seg_data.ljust(CHUNK_SZ, b'\x00')
        seed = gf.wallet_seed(first_block)
        checksum = gf.xxh64(seg_data) & 0xFFFF
        fast_sig = seg_data[0] if seg_data else 0

        coord = struct.pack('<I', si)
        coord += struct.pack('<B', 0)
        coord += struct.pack('<B', si % 12)
        coord += struct.pack('<B', si % 6)
        coord += struct.pack('<B', 0)
        coord += struct.pack('<B', first_strategy)
        coord += struct.pack('<Q', seed)
        coord += struct.pack('<H', checksum)
        coord += struct.pack('<B', fast_sig)
        buf += coord

    # LetterCube section
    for lc_buf in lc_states:
        buf += lc_buf

    # Segment data
    for seg_data, _ in segments:
        buf += struct.pack('<I', len(seg_data))
        buf += seg_data

    stats['lc_section_sz'] = lc_section_sz
    return bytes(buf), stats


def roundtrip_decode(encoded):
    """Decode with LetterCube verification."""
    buf = memoryview(encoded)

    # Header
    magic = bytes(buf[0:4])
    assert magic == GPXL_MAGIC, f"Bad magic: {magic}"
    n_segments = struct.unpack_from('<I', buf, 10)[0]
    original_size = struct.unpack_from('<Q', buf, 14)[0]
    stored_xxh64 = struct.unpack_from('<Q', buf, 22)[0]
    coord_lc_sz = struct.unpack_from('<I', buf, 30)[0]

    # Verify LetterCube states
    coord_section_sz = 20 * n_segments
    lc_section_sz = coord_lc_sz - coord_section_sz
    actual_lc_count = lc_section_sz // LC_BUF

    lc_verify_count = 0
    lc_start = GPXL_HEADER_SZ + coord_section_sz
    for ci in range(actual_lc_count):
        lc_offset = lc_start + ci * LC_BUF
        lc_bytes = bytes(buf[lc_offset:lc_offset + LC_BUF])
        if gf._lib.geofield_lc_verify(lc_bytes):
            lc_verify_count += 1

    data_offset = GPXL_HEADER_SZ + coord_lc_sz

    # Read segments
    result = bytearray()
    pos = data_offset
    for si in range(n_segments):
        seg_len = struct.unpack_from('<I', buf, pos)[0]
        pos += 4
        seg_data = bytes(buf[pos:pos + seg_len])
        pos += seg_len
        result += seg_data

    result = bytes(result[:original_size])
    actual_xxh64 = gf.xxh64(result)
    ok = (actual_xxh64 == stored_xxh64)

    return result, ok, lc_verify_count


def test_roundtrip(path):
    """Test encode -> decode -> verify."""
    with open(path, 'rb') as f:
        original = f.read()
    original_hash = hashlib.sha256(original).hexdigest()
    name = os.path.basename(path)

    t0 = time.perf_counter()
    encoded, stats = roundtrip_encode(original)
    t1 = time.perf_counter()
    decoded, ok, lc_ok = roundtrip_decode(encoded)
    t2 = time.perf_counter()

    decoded_hash = hashlib.sha256(decoded).hexdigest()
    match = (original_hash == decoded_hash)
    ratio = len(encoded) / len(original) if len(original) > 0 else 999

    print(f"\n{name} ({len(original):,} bytes)")
    print(f"  Segments: {stats['n_segments']}  Blocks: {stats['total_blocks']}")
    print(f"  LetterCube: {lc_ok}/{stats['n_segments']} verified")
    print(f"  Skeleton: ID={stats['skel_hits'][0]} FLAT={stats['skel_hits'][1]} "
          f"DIFF={stats['skel_hits'][2]} BREF={stats['skel_hits'][3]} "
          f"GEOM={stats['skel_hits'][4]} RAW={stats['skel_hits'][5]}")
    print(f"  Diamond: FLAT={stats['shell_hits'][0]} SPARSE={stats['shell_hits'][1]} DENSE={stats['shell_hits'][2]}")
    print(f"  Encoded: {len(encoded):,} bytes  ratio={ratio:.2f}x")
    print(f"  Encode: {(t1-t0)*1000:.1f}ms  Decode: {(t2-t1)*1000:.1f}ms")
    print(f"  xxh64 match: {ok}  SHA256 match: {match}")

    if not match:
        print(f"  ORIGINAL hash: {original_hash}")
        print(f"  DECODED  hash: {decoded_hash}")
        for i in range(min(len(original), len(decoded))):
            if original[i] != decoded[i]:
                print(f"  First diff at byte {i}: orig=0x{original[i]:02x} decoded=0x{decoded[i]:02x}")
                break

    return match


if __name__ == '__main__':
    test_files = [
        '../tools/geopixel_pipeline.py',
        '../runner/llama_pogls_runner_sid_v2.c',
        '../core/geo_field_core.h',
        '../collection/geopixel/hbv_bundle/geo_flow_chunker_v8.h',
    ]

    print("=" * 65)
    print("Phase A4+B1: Adaptive Chunk + LetterCube Roundtrip Test")
    print("=" * 65)

    all_pass = True
    for path in test_files:
        full = os.path.join(os.path.dirname(__file__), path)
        if os.path.exists(full):
            ok = test_roundtrip(full)
            if not ok:
                all_pass = False

    print("\n" + "=" * 65)
    print(f"RESULT: {'ALL PASS' if all_pass else 'SOME FAILED'}")
    print("=" * 65)
