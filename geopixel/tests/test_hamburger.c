...
static void t4_invert(void) {
    ...
    hb_invert_record_enc(&rec, 0, orig0, enc0, 8u, 8u, 0u, 0u);
...
static void t5_roundtrip(void) {
    ...
    const char *tmpfile = ".\\temp\\test_hamburger.gpx5";
...
static void t8_encode_image(void) {
    ...
    int r = hamburger_encode_image(".\\temp\\test_img.gpx5", &img, 0xDEADBEEFu, 0);
    CHECK("encode_image returns OK", r == HB_OK);

    /* verify file exists and has correct magic */
    FILE *f = fopen(".\\temp\\test_img.gpx5","rb");
    ...
    uint32_t nt=0, tsz=0;
    r = hamburger_decode(".\\temp\\test_img.gpx5",(uint8_t**)out,&nt,&tsz);
...
static void t9_decode_image(void) {
    ...
    int r = hamburger_encode_image(".\\temp\\t9_img.gpx5", &img, 0xCAFEBABEu, 0);
    CHECK("T9 encode_image OK", r == HB_OK);
    if (r != HB_OK) return;

    HbImageOut dec;
    memset(&dec, 0, sizeof(dec));
    r = hamburger_decode_image(".\\temp\\t9_img.gpx5", &dec, W, H, TW, TH);
