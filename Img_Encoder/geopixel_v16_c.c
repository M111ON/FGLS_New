/*
 * GeoPixel v16 — Adaptive LOOSE window fix (was hardcode 96 → now 160+tile_var)
 *
 * vs v15: GRAD9_LOOSE was near 0% because thresh_loose=96 < thresh_tight when tile_var>96
 *   Fix: thresh_loose = 160 + tile_var  (always > thresh_tight = 48 + tile_var>>1)
 *   Expected: GRAD9_L 0% → ~20-30%,  DELTA/FALLBACK 22% → ~10-15%
 *   Ratio: ~3.16x → ~3.3-3.5x
 *
 * New: atomic grad mode counters (g_count_grad_normal/loose/none) for diagnostics
 *
 * Compile: gcc -O3 -o geopixel_v16 geopixel_v16_c.c -lm -lzstd -lpthread
 * Usage  : ./geopixel_v16 input.bmp | file.gp16 | file.gp16 tx ty
 */
        ...
        int *out_size)
{
    ...
    if(ttype==TTYPE_GRADIENT){
        ...
        int thresh_tight = 48 + (tile_var >> 1);  /* adaptive strict window */
        if(thresh_tight > 80) thresh_tight = 80;   /* cap: prevent LOOSE starvation */
        int thresh_loose = 160 + tile_var;         /* adaptive loose window (always > tight) */
        int grad_mode;
...
static int encode(const char *path){
    ...
    int cg = tc[1];  /* total gradient tiles */
    ...
    printf("\n=== Grad mode (of %d gradient tiles) ===\n", cg);
    printf("  GRAD9_N : %4d  (%5.1f%%)\n", cn, cg>0?(double)cn/cg*100.0:0.0);
    printf("  GRAD9_L : %4d  (%5.1f%%)\n", cl, cg>0?(double)cl/cg*100.0:0.0);
    printf("  FALLBACK: %4d  (%5.1f%%)\n", cd, cg>0?(double)cd/cg*100.0:0.0);
    printf("\n=== Results ===\n");
    printf("  Raw      : %7d B\n",raw);
