...
// returns: anchor(h) + stream(sn) + residual(sn) in single buffer
// layout: [anchors:h][stream:sn][residual:sn]
uint8_t* encode_plane(const uint8_t *plane, int w, int h, int ch, int *out_sz) {
    ...
    *out_sz = h + sn + sn;
    ...
    for(int y=0;y<h;y++){
        ...
        for(int x=1;x<w;x++){
            int idx=y*(w-1)+(x-1);
            int raw_d=(int)plane[y*w+x]-(int)plane[y*w+x-1]; int8_t d=(int8_t)(raw_d);
            uint8_t sp,zo; d2cyl(d,ch,&sp,&zo);
...
int main(int argc, char **argv){
    const char *path=argc>1?argv[1]:"test01.bmp";
    ...
    for(int c=0;c<3;c++){
        ...
        int8_t  *resid=(int8_t*)(enc[c]+h+sn);
        ...
        printf("  [%s] stream=%.3fb  residual=%.3fb(%duniq)  theo=%dB\n",
               pname[c],es,er,uniq,h+ts+tr);
        total_theo+=h+ts+tr;
    }
    ...
    printf("  PNG          : ~393847 B  (2.00x)\n");
    printf("  Plane verify : %s\n",all_ok?"PASS":"FAIL");
    printf("  PSNR         : %.2f dB  (only lossy from clip=%.2f%%)\n",
