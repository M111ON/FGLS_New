...
/* adaptive effective thresh = GRAD_ERR_TIGHT + (tile_var >> 2) */
/* force GRAD9 when tile_var below this — near-flat gradient */
#define GRAD_VAR_FORCE  64
        ...
        int *out_size)
{
    ...
    if(ttype==TTYPE_GRADIENT){
        ...
        int thresh_tight = GRAD_ERR_TIGHT + (tile_var >> 2);  /* adaptive */
        int grad_mode;
    ...
    /* loose bias: per-tile gerr proxy — use Y grad error as bias source */
    ...
    if(is_loose){
        ...
        int gerrCg = compute_grad_error(iCg,x0,y0,x1,y1,W,grad_Y0Cg,grad_dxCg,grad_dyCg);
        int gerrCo = compute_grad_error(iCo,x0,y0,x1,y1,W,grad_Y0Co,grad_dxCo,grad_dyCol);
        loose_bias_Y  = gerrY  >> 3;
        loose_bias_Cg = gerrCg >> 3;
        loose_bias_Co = gerrCo >> 3;
    }
    ...
    if(is_loose){
        ...
        blob[off++]=(uint8_t)(loose_bias_Cg & 0xFF);
        blob[off++]=(uint8_t)(loose_bias_Co & 0xFF);
    }
        ...
        uint8_t *px_out)
{
    ...
    /* read bias bytes written by encoder — exact, no derivation */
    int loose_bias_Y=0, loose_bias_Cg=0, loose_bias_Co=0;
    if(is_loose){
        loose_bias_Y  = (int)blob[off++];
        loose_bias_Cg = (int)blob[off++];
        loose_bias_Co = (int)blob[off++];
    }
    int def_Y=128,def_C=0;
