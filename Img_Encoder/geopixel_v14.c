/*
 * GeoPixel v14 — Parametric Boundary Model (BMODE_PARAM for EDGE tiles)
 *
 * vs v13:
 *   EDGE boundary: DELTA ~195B → PARAM 32B  (↓6x per tile)
 *   Model: val(x,y) ≈ Y0 + dY_dx*x + dY_dy*y + curve_x*x^2 + curve_y*y^2 + hint
 *   Per channel (10B): Y0(int16) dY_dx(int8) dY_dy(int8)
 *                      curve_x(int8) curve_y(int8) hint[4](int8×4)
 *   Total: 10B×3ch + 2B flags = 32B
 *
 * Boundary budget per tile:
 *   FLAT:     0B
 *   GRADIENT: 18B  (LINEAR, unchanged)
 *   EDGE:     32B  (PARAM, new — was ~195B)
 *   NOISE:   ~195B (DELTA, unchanged — accuracy first)
 *
 * Compile: gcc -O3 -o geopixel_v14 geopixel_v14.c -lm -lzstd -lpthread
 * Usage  : ./geopixel_v14 input.bmp
 *          ./geopixel_v14 file.gp14
 *          ./geopixel_v14 file.gp14 tx ty
 */
...
/* ── config ─────────────────────────────────────────────── */
#define MAGIC_V13   0x47503D77u  /* v14 magic */
#define TILE        32
#define ZST_LVL     9
...
#define BMODE_DELTA   2   /* NOISE: delta-coded full boundary */
#define BMODE_PARAM   3   /* EDGE: parametric 32B model (new v14) */

/* BMODE_PARAM layout per channel (10B × 3ch + 2B flags = 32B):
 *   int16  Y0        — corner value (zigzag)
 *   int8   dY_dx     — gradient x  (Q4 fixed: divide by 16 in decode)
 *   int8   dY_dy     — gradient y
 *   int8   curve_x   — curvature x (Q6)
 *   int8   curve_y   — curvature y
 *   int8   hint[4]   — residual correction at (8,0)(24,0)(0,8)(0,24)
 *   ─────────────────────────────────────
 *   uint8  mode_flags — bit0=has_curve, bit1=has_hints
 *   uint8  reserved
 */
...
/*
 * Parametric boundary (BMODE_PARAM) — 10B per channel, 32B total.
 * Fits Y0 + linear(dx,dy) + curvature(cx,cy) + 4 residual hints.
 * Reconstruction: val(x,y) = Y0 + dx*x/16 + dy*y/16
 *                           + cx*(x-tw/2)/64 + cy*(y-th/2)/64 + hint_interp
 * Hints placed at x=tw/4,3tw/4 (y=0) and y=th/4,3th/4 (x=0).
 */
static void encode_boundary_param(
        const int *col, const int *row, int corner,
        int th, int tw, Buf *out)
{
    /* fit linear gradient: least-squares slope */
    int N_col=th, N_row=tw;
    /* Y0 = corner */
    /* dx = (row[tw-1] - row[0]) / (tw-1), Q4 */
    ...
    /* curvature: fit quadratic residual at midpoints */
    int mid_x = tw/2, mid_y = th/2;
    int pred_mid_col = corner + (dy * mid_y / 16);
    int pred_mid_row = corner + (dx * mid_x / 16);
    int actual_mid_col = (th>1) ? col[mid_y] : corner;
    int actual_mid_row = (tw>1) ? row[mid_x] : corner;
    /* curve ≈ residual_at_mid / (mid^2) * 64, clamped int8 */
    int cx = mid_x>0 ? ((actual_mid_row - pred_mid_row) * 64 / (mid_x)) : 0;
    int cy = mid_y>0 ? ((actual_mid_col - pred_mid_col) * 64 / (mid_y)) : 0;
    if(cx< -127) cx=-127; if(cx>127) cx=127;
    if(cy< -127) cy=-127; if(cy>127) cy=127;

    /* 4 residual hints: row[tw/4], row[3*tw/4], col[th/4], col[3*th/4] */
    int hx0=(tw>3)?tw/4:0, hx1=(tw>3)?3*tw/4:(tw-1);
    int hy0=(th>3)?th/4:0, hy1=(th>3)?3*th/4:(th-1);
    /* predicted values at hint positions */
    int ph0 = corner + dx*hx0/16 + cx*(hx0-mid_x)/64;
    int ph1 = corner + dx*hx1/16 + cx*(hx1-mid_x)/64;
    int ph2 = corner + dy*hy0/16 + cy*(hy0-mid_y)/64;
    int ph3 = corner + dy*hy1/16 + cy*(hy1-mid_y)/64;
    int hint[4];
    hint[0] = (tw>3) ? (row[hx0]-ph0) : 0;
    hint[1] = (tw>3) ? (row[hx1]-ph1) : 0;
    hint[2] = (th>3) ? (col[hy0]-ph2) : 0;
    hint[3] = (th>3) ? (col[hy1]-ph3) : 0;
    for(int i=0;i<4;i++){
        if(hint[i]< -127) hint[i]=-127;
        if(hint[i]>  127) hint[i]= 127;
    }

    bpz(out,(int16_t)corner);
    bp(out,(uint8_t)(int8_t)dx);
    bp(out,(uint8_t)(int8_t)dy);
    bp(out,(uint8_t)(int8_t)cx);
    bp(out,(uint8_t)(int8_t)cy);
    for(int i=0;i<4;i++) bp(out,(uint8_t)(int8_t)hint[i]);
    /* 2+1+1+1+1+4 = 10B */
    (void)N_col; (void)N_row;
}

static void decode_boundary_param(
        const uint8_t *blob, int *off,
        int *col_out, int *row_out, int *corner_out,
        int th, int tw)
{
    ...
    int cx  = (int)(int8_t)blob[(*off)++];
    int cy  = (int)(int8_t)blob[(*off)++];
    int hint[4];
    for(int i=0;i<4;i++) hint[i]=(int)(int8_t)blob[(*off)++];
    ...
    *corner_out = corner;
    ...
    /* reconstruct left column */
    for(int y=0;y<th;y++){
        int v = corner + dy*y/16 + cy*(y-mid_y)/64;
        /* linear interpolate hint correction */
        if(th>3){
            int corr=0;
            if(y<=hy0)        corr = hint[2]*y/hy0;
            else if(y<=hy1)   corr = hint[2] + (hint[3]-hint[2])*(y-hy0)/(hy1-hy0);
            else              corr = hint[3] + (0-hint[3])*(y-hy1)/(th-1-hy1);
            v += corr;
        }
        col_out[y]=v;
    }
    /* reconstruct top row */
    for(int x=0;x<tw;x++){
        int v = corner + dx*x/16 + cx*(x-mid_x)/64;
        if(tw>3){
            int corr=0;
            if(x<=hx0)        corr = hint[0]*x/hx0;
            else if(x<=hx1)   corr = hint[0] + (hint[1]-hint[0])*(x-hx0)/(hx1-hx0);
            else              corr = hint[1] + (0-hint[1])*(x-hx1)/(tw-1-hx1);
            v += corr;
        }
        row_out[x]=v;
    }
        ...
        int *out_size)
{
    ...
    /* boundary mode by type — v14: EDGE uses PARAM (32B) */
    int bmode;
    if(ttype==TTYPE_GRADIENT) bmode=BMODE_LINEAR;
    else if(ttype==TTYPE_EDGE) bmode=BMODE_PARAM;
    else bmode=BMODE_DELTA;  /* NOISE: keep delta for accuracy */
    ...
    } else if(bmode==BMODE_PARAM){
        encode_boundary_param(col_Y, row_Y, adj_tl_Y, th,tw,&bctx);
        encode_boundary_param(col_Cg,row_Cg,adj_tl_Cg,th,tw,&bctx);
        encode_boundary_param(col_Co,row_Co,adj_tl_Co,th,tw,&bctx);
    } else {
    ...
    if(bmode==BMODE_LINEAR){
        ...
        decode_boundary_param(bctx.buf,&tmp_off,ctxLY, ctxTY, &dummy_tl_Y, th,tw);
        decode_boundary_param(bctx.buf,&tmp_off,ctxLCg,ctxTCg,&dummy_tl_Cg,th,tw);
        decode_boundary_param(bctx.buf,&tmp_off,ctxLCo,ctxTCo,&dummy_tl_Co,th,tw);
    } else {
        ...
        uint8_t *px_out)
{
    ...
    if(bmode==BMODE_LINEAR){
        ...
        decode_boundary_param(blob,&off,ctxLY, ctxTY, &tl_Y, th,tw);
        decode_boundary_param(blob,&off,ctxLCg,ctxTCg,&tl_Cg,th,tw);
        decode_boundary_param(blob,&off,ctxLCo,ctxTCo,&tl_Co,th,tw);
    } else {
...
static int query_tile(const char *path,int qtx,int qty){
    ...
    const char *bmnames[]={"NONE","LINEAR","DELTA","PARAM"};
