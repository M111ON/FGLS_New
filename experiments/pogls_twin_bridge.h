...
/* LC Gate — LetterCube routing layer (Frustum DNA) */
    ...
    PipelineResult *res_out)
{
    ...
    /* ── LC gate: Frustum routing decision ──────────────────────────
     * GROUND  → bypass geo_fast_intersect entirely, drop to dodeca
     * COLLISION → turbulence bump on drift_acc before intersect
     * WARP / ROUTE → continue to geo_fast_intersect normally        */
    LCTwinDecision lc_dec = lc_twin_gate(&b->lc_gate, addr, value, raw);
    ...
    /* LC COLLISION → turbulence bump before intersect */
...
static inline TwinBridgeStats twin_bridge_stats(const TwinBridge *b)
{
    ...
    s.write_density = b->total_ops > 0
                    ...
                    : 0.0f;
    s.lc_warp      = b->lc_gate.warp_count;
    s.lc_route     = b->lc_gate.route_count;
    s.lc_ground    = b->lc_gate.ground_count;
    s.lc_collision = b->lc_gate.collision_count;
    return s;
}
