...
#define GPX4_GEO_ADDR_SZ      4   /* per tile: 4B packed TileAddr      */

/* GEO layer name tag */
#define GPX4_GEO_NAME        "GEOA"  /* 4-char tag for LAYER_GEO         */
...
/* ══════════════════════════════════════════════════════
 * GEO LAYER — O21 Pentagon Address Table
 *   Layer type : GPX4_LAYER_GEO (0x06)
 *   Layer name : "GEOA"
 *   No tile table (untiled, like META/ANIM_HDR)
 *   Layer data : n_tiles x 4B  (big-endian uint32 packed)
 *
 *   packed layout (32-bit):
 *     [31..28]  pent_id      (4b, 0-11 icosa pentagon)
 *     [27..14]  hilbert_local(14b, texture-flow order within cell)
 *     [13.. 0]  sub          (14b, reserved/balance-refine flag in bit 13)
 *
 *   One GEO layer covers ALL tiles for that frame.
 *   For animation: one GEO layer per keyframe, or share a single
 *   GEO layer across the whole file (geometry is image-independent).
 * ══════════════════════════════════════════════════════ */
...
/* Convenience macros to unpack fields */
#define GPX4_GEO_PENT(p)    (((p)>>28)&0x0Fu)
#define GPX4_GEO_HILBERT(p) (((p)>>14)&0x3FFFu)
#define GPX4_GEO_SUB(p)     ((p)&0x3FFFu)

/*
 * gpx4_geo_write_layer_data — serialise n_tiles GeoAddr -> raw bytes (big-endian)
 *   buf must be pre-allocated: n_tiles * GPX4_GEO_ADDR_SZ bytes
 */
static inline void gpx4_geo_write_layer_data(
        const Gpx4GeoAddr *addrs, uint32_t n_tiles, uint8_t *buf)
{
    for(uint32_t i = 0; i < n_tiles; i++)
        g4w4(buf + i * GPX4_GEO_ADDR_SZ, addrs[i].packed);
}

/*
 * gpx4_geo_read_layer_data — deserialise raw bytes -> GeoAddr array
 */
static inline void gpx4_geo_read_layer_data(
        const uint8_t *buf, uint32_t n_tiles, Gpx4GeoAddr *out)
{
    for(uint32_t i = 0; i < n_tiles; i++)
        out[i].packed = g4r4(buf + i * GPX4_GEO_ADDR_SZ);
}
...
static inline int gpx4_open(const char *path, Gpx4File *gf)
{
    ...
    for(int i = 0; i < nl; i++){
        ...
        if(t == GPX4_LAYER_META || t == GPX4_LAYER_ANIM_HDR || t == GPX4_LAYER_GEO) continue;
...
/*
 * gpx4_geo_open — find "GEOA" layer in open Gpx4File,
 *   malloc + deserialise. Returns NULL if absent. Caller frees.
 */
static inline Gpx4GeoAddr* gpx4_geo_open(const Gpx4File *gf)
{
    uint32_t sz = 0;
    const uint8_t *buf = gpx4_layer_data_by_name(gf, GPX4_GEO_NAME, &sz);
    if(!buf) return NULL;
    uint32_t expected = gf->n_tiles * GPX4_GEO_ADDR_SZ;
    if(sz < expected) return NULL;
    Gpx4GeoAddr *out = (Gpx4GeoAddr*)malloc(expected);
    if(!out) return NULL;
    gpx4_geo_read_layer_data(buf, gf->n_tiles, out);
    return out;
}
