...
/* O23_HILBERT_N: legacy constant — actual n is computed per-image in o23_build_geo_addrs */
#define O23_HILBERT_N 16  /* kept for reference; not used in hilbert calls anymore */

/* next power-of-2 >= v */
static uint32_t o23_next_pow2(uint32_t v){
    if(v==0) return 1;
    v--;
...
static void o23_build_geo_addrs(int W,int H,int TW,int TH,Gpx4GeoAddr *out){
    ...
    /* hilbert grid must be power-of-2 and >= max(TW,TH) to avoid OOB rotation */
    uint32_t hn = o23_next_pow2((uint32_t)(TW > TH ? TW : TH));
    if(hn < 2) hn = 2;
    ...
    for(int ty=0;ty<TH;ty++){
        for(int tx=0;tx<TW;tx++){
            ...
            assert((uint32_t)tx < hn && (uint32_t)ty < hn); /* hilbert OOB guard */
            uint32_t hd=o23_hilbert(hn,(uint32_t)tx,(uint32_t)ty);
...
#define DEDUP_TABLE_SIZE  4096
     ...
     * ──────────────────────────────────────────────────────────────────── */
    {
        /* Pass 1: count O4 rows — skip TTYPE_NOISE tiles */
        ...
        for(int i=0; i<NT && gpx_rgb; i++){
            ...
            int is_dedup_ref = (off_raw & 0x80000000u) != 0;
            if(bsz == 0 && jobs[i].ttype != TTYPE_NOISE)
                fprintf(stderr,"[DEDUP_BUG] tile=%d ttype=%d is_dedup=%d off=0x%08x bsz=0\n",
                        i, jobs[i].ttype, is_dedup_ref, off_raw);
            if(tile_bsz) tile_bsz[i] = bsz;

            /* ── Noise: bypass O4, store raw blob in fallback section ── */
            if(jobs[i].ttype == TTYPE_NOISE){
                if(tile_rows) tile_rows[i] = 0;  /* sentinel */
        ...
        if(gpx_rgb){
            ...
            for(int i=0; i<NT; i++){
                if(tile_rows[i] > 0 && tile_bsz[i] > 0){
                    /* O4 tile */
                    o4_tiles[i].rows    = tile_rows[i];
                    o4_tiles[i].tflags  = GPX4_TILE_PRESENT;
                    o4_tiles[i].blob_sz = tile_bsz[i];
                } else {
                    /* noise or degenerate (rows>0 but blob_sz==0) → fallback */
                    if(tile_rows[i] > 0 && tile_bsz[i] == 0)
                        fprintf(stderr,"gpx4 enc warn: tile %d rows=%u blob_sz=0, demote to noise\n", i, tile_rows[i]);
                    fb_tiles[i].rows    = 0;
            ...
            int meta_len = snprintf(meta_buf, sizeof(meta_buf),
                "o4_rows=%u,noise=%d,o4_rt=%s,grid_w=%u",
                total_rows, n_fallback,
                o4_failed?"FAIL":"OK", O4_GRID_W);
...
static int decode_gpx4(const char *gpx_path, const char *out_bmp){
    ...
    if(fb_data && fb_tiles){
        ...
        for(int i=0; i<NT; i++){
            fb_offsets[i] = fb_cur;
            if(o4_tiles[i].rows == 0) fb_cur += fb_tiles[i].blob_sz;
        }
    }
    ...
    for(int k=0; k<NT; k++){
        ...
        if(blob_sz == 0){
            fprintf(stderr,"gpx4 warn: tile %d rows=%u blob_sz=0, skip\n", i, gh);
            continue;
        }
        uint32_t n_chunks = (blob_sz + O4_CHUNK_BYTES - 1) / O4_CHUNK_BYTES; /* = ceil(blob_sz/3) */
        ctx.n_slots = n_chunks;
        uint8_t *blob = (uint8_t*)malloc(blob_sz);
        o4_decode(&ctx, blob, n_chunks, blob_sz);
...
static int bench_gpx4(const char *gpx_path){
    ...
    #define O24_DECODE_LOOP(ORD) do { \
        for(int k=0;k<NT;k++){ \
            ...
            if(bsz==0){fprintf(stderr,"gpx4 warn: tile %d rows=%u blob_sz=0, skip\n",i,gh);break;} \
...
static int decode_gpx(const char *gpx_path, const char *out_bmp){
    ...
    for(int i=0; i<NT; i++){
        ...
        uint32_t n_slots = (blob_sz + O4_CHUNK_BYTES - 1) / O4_CHUNK_BYTES; /* ceil(blob_sz/3) */
        ctx.n_slots = n_slots;
        uint8_t *blob = (uint8_t*)malloc(blob_sz);
