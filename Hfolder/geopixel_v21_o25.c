...
/* predictors */
#define PRED_MED  0   /* median(L,T,L+T-TL)       */
#define PRED_GRAD 1   /* gradient: L+T-TL          */
#define PRED_LEFT 2   /* copy left                 */
#define PRED_TOP  3   /* copy top                  */
#define PRED_AVG  4   /* (L+T)/2                   */
#define N_PRED    5
        ...
        int x0,int y0,int x1,int y1,int W,int *out_pred)
{
    ...
    /* O25: variance per predictor — all 5 pids, all 3 channels, no heap alloc
     * var[] accumulates sum-of-squares of residuals across Y+Cg+Co.
     * Single pixel scan: compute L/T/TL once, apply all 5 predictors inline.
     */
    long long var[N_PRED]={0};
    for(int y=y0;y<y1;y++) for(int x=x0;x<x1;x++){
        /* Y channel neighbours */
        int LY =x>x0?iY[y*W+x-1]  :(x>0?iY[y*W+x-1]  :128);
        int TY =y>y0?iY[(y-1)*W+x]:(y>0?iY[(y-1)*W+x]:128);
        int TLY=(x>x0&&y>y0)?iY[(y-1)*W+x-1]:(x>0&&y>0?iY[(y-1)*W+x-1]:128);
        int vY=iY[y*W+x];  /* Y-only for predictor selection variance */
        for(int pid=0;pid<N_PRED;pid++){
            int pY;
            switch(pid){
                case PRED_MED:{
                    int hiY=LY>TY?LY:TY,loY=LY<TY?LY:TY;
                    pY=TLY>=hiY?loY:TLY<=loY?hiY:LY+TY-TLY; break;}
                case PRED_GRAD: pY=LY+TY-TLY; break;
                case PRED_LEFT: pY=LY; break;
                case PRED_TOP:  pY=TY; break;
                default: /* PRED_AVG */ pY=(LY+TY)>>1; break;
            }
            /* Y-only variance for threshold calibration consistency */
            int rY=vY-pY;
            var[pid]+=(long long)rY*rY;
        }
    }
    int best=0; for(int p=1;p<N_PRED;p++) if(var[p]<var[best]) best=p;
    *out_pred=best;
    double avg_var=(double)var[best]/tn;       /* Y-only variance / pixel — original calibration */
    ...
    if(avg_var<1000.0)    return TTYPE_EDGE;   /* BMODE_DELTA auto for high-var */
    /* noise histogram probe (avg_var >= 1000) */
...
#define DEDUP_TABLE_SIZE  4096
    ...
    int tc[5]={0};
    /* O25 validation metrics */
    int bmc[4]={0};          /* bmode counts: NONE/LINEAR2/DELTA/GRAD9 */
    long long total_var=0; int var_count=0;
    for(int i=0;i<NT;i++){
        int t=jobs[i].ttype; if(t<0||t>4) t=3;
        tc[t]++;
        /* bmode is blob[1] for non-FLAT tiles */
        if(jobs[i].blob && jobs[i].blob_sz>1 && t!=TTYPE_FLAT){
            int bm=jobs[i].blob[1]; if(bm<0||bm>3) bm=3;
            bmc[bm]++;
        }
        /* avg_var proxy: sum residual stream sizes (hi[Y] stream = blob stream 0) */
        if(jobs[i].blob && jobs[i].blob_sz>0 && t!=TTYPE_FLAT){
            /* stream 0 size is at fixed offset: 4+0+6+(bctx_sz) — approximate via blob_sz */
            total_var += jobs[i].blob_sz; var_count++;
        }
    }
...
#define FB_BUCKET_DEPTH    8
        ...
        for(int i=0; i<NT && gpx_rgb; i++){
            ...
            /* ── O4 path: FLAT / GRADIENT / EDGE / DELTA ── */
            O4GridCtx ctx;
    ...
    /* O25 validation metrics */
    int n_pred = tc[1]+tc[2]+tc[3]; if(n_pred<1)n_pred=1;
    printf("\n=== O25 Predictor validation ===\n");
    printf("  Avg blob/tile (predictive) : %.1f B\n", var_count>0?(double)total_var/var_count:0.0);
    printf("  BMODE usage (of %d pred tiles):\n", n_pred);
    printf("    NONE     : %4d\n", bmc[0]);
    printf("    LINEAR2  : %4d  (%4.1f%%)\n", bmc[1], (double)bmc[1]/n_pred*100.0);
    printf("    DELTA    : %4d  (%4.1f%%)  ← key metric\n", bmc[2], (double)bmc[2]/n_pred*100.0);
    printf("    GRAD9    : %4d  (%4.1f%%)\n", bmc[3], (double)bmc[3]/n_pred*100.0);
    printf("  Noise%%     : %.1f%%  (target < 20%%)\n", (double)tc[3]/NT*100.0);
