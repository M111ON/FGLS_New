...
/* Point 3: Goldberg override counters */
static atomic_int g_count_gb_override_loose = 0;  /* gerr=NORMAL → GB→LOOSE */
static atomic_int g_count_gb_override_delta = 0;  /* gerr=LOOSE  → GB→DELTA */
...
#define DEDUP_TABLE_SIZE  4096
    ...
    /* ── O5: GPX output — O4 grid → single PNG ───────────────────────────
     * Each tile blob (from C buffer) → o4_encode → 27×H RGB rows
     * Tiles stacked vertically → one PNG file (W=27, H=total_rows)
     * GPX = lossless geometric representation of all tile data
     * ──────────────────────────────────────────────────────────────────── */
    {
        /* Pass 1: count total rows needed — read from C buffer (idx already freed) */
        ...
        /* Allocate RGB buffer for full PNG: W=27, H=total_rows */
        uint32_t gpx_rgb_sz = O4_GRID_W * total_rows * 3u;
        uint8_t *gpx_rgb = (uint8_t*)calloc(gpx_rgb_sz, 1);
        uint32_t row_cursor = 0;
        int o4_failed = 0;

        /* Per-tile row counts — needed by decode path */
        uint16_t *tile_rows = (uint16_t*)malloc(NT * sizeof(uint16_t));
        uint32_t *tile_bsz  = (uint32_t*)malloc(NT * sizeof(uint32_t));

        /* Pass 2: encode each tile blob → O4 grid → write rows */
        for(int i=0; i<NT && gpx_rgb; i++){
            uint32_t off_raw = r32(C, HDR_BYTES + i*8);
            uint32_t bsz     = r32(C, HDR_BYTES + i*8 + 4);
            /* dedup tiles point to same offset — still encodeable */
            ...
            /* convert grid → interleaved RGB rows */
            uint8_t row_rgb[O4_GRID_W * 3];
        ...
        /* Write GPX file: 16B header + PNG stream */
        if(gpx_rgb){
            char gpx_path[512]; snprintf(gpx_path,sizeof(gpx_path),"%s.gpx",path);
            png_structp pp = png_create_write_struct(PNG_LIBPNG_VER_STRING,NULL,NULL,NULL);
            png_infop   pi = png_create_info_struct(pp);
            if(!setjmp(png_jmpbuf(pp))){
                FILE *gf = fopen(gpx_path,"wb");
                /* GPX2 header: magic(4) + TW(2) + TH(2) + NT(4) + total_rows(4) = 16B
                 * NT×6B table: [2B grid_h][4B blob_sz] per tile → PNG at 16+NT*6 */
                uint8_t hdr16[16]={0x47,0x50,0x58,0x32,  /* GPX2 */
                    (uint8_t)(TW>>8),(uint8_t)TW,
                    (uint8_t)(TH>>8),(uint8_t)TH,
                    (uint8_t)(NT>>24),(uint8_t)(NT>>16),(uint8_t)(NT>>8),(uint8_t)NT,
                    (uint8_t)(total_rows>>24),(uint8_t)(total_rows>>16),
                    (uint8_t)(total_rows>>8),(uint8_t)total_rows};
                fwrite(hdr16, 1, 16, gf);
                /* tile table: NT × 6B [rows:2][bsz:4] big-endian */
                if(tile_rows && tile_bsz){
                    for(int ti=0; ti<NT; ti++){
                        uint32_t bs = tile_bsz[ti];
                        uint8_t rec[6]={
                            (uint8_t)(tile_rows[ti]>>8),(uint8_t)tile_rows[ti],
                            (uint8_t)(bs>>24),(uint8_t)(bs>>16),(uint8_t)(bs>>8),(uint8_t)bs};
                        fwrite(rec, 1, 6, gf);
                    }
                }
                png_init_io(pp, gf);
                png_set_IHDR(pp,pi, O4_GRID_W, total_rows, 8,
                ...
                printf("\n=== O5 GeoPixel Grid (.gpx) ===\n");
                printf("  Tiles     : %d  grid=%dx%d\n", NT, O4_GRID_W, total_rows);
                printf("  Roundtrip : %s\n", o4_failed?"FAIL ✗":"OK ✓");
                printf("  GPX size  : %ld B  (%.2fx vs gp15)\n", gpx_sz,
...
/* ════════════════════════════════════════════════════════
 * O6: DECODE GPX
 * .gpx → reconstruct pixels via O4 grid → tile blobs → image
 * Format: 16B header + NT×2B row_counts + PNG(27×total_rows)
 * ════════════════════════════════════════════════════════ */
static int decode_gpx(const char *gpx_path, const char *out_bmp){
    /* ── 1. Read raw file ── */
    FILE *f = fopen(gpx_path, "rb");
    if(!f){ fprintf(stderr,"gpx: cannot open %s\n", gpx_path); return 1; }
    ...
    /* ── 2. Parse 16B header ── */
    if(fsz < 16 || raw[0]!='G'||raw[1]!='P'||raw[2]!='X'||raw[3]!='2'){
    ...
    uint8_t *gpx_rgb = (uint8_t*)malloc(O4_GRID_W * total_rows * 3u);
    {
        /* wrap raw+png_offset in FILE* via fmemopen */
        FILE *mf = fmemopen(raw + png_offset, (size_t)(fsz - png_offset), "rb");
