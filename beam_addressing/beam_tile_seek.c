/*
 * beam_tile_seek.c — Tessellation + Frame Seek Tile
 *
 * Principle: tessellation gives accurate position,
 *            frame_seek gives compact tile (1440 positions).
 *
 * Flow:
 *   weight → tessellation node (20736 grid, accurate)
 *   node → clock_tick (1440 timeline, compact)
 *   clock_tick → frame_seek tile (stride-37, O(1))
 *
 * Storage: tile per weight (11 bits for 1440 tiles)
 * Decode: tile → weight via LUT (256 entries)
 *
 * Core functions from:
 *   geo_jump.h: geo_clock_tick(node_id) = (node_id × 1440) / 20736
 *   geo_frame_seek.h: frame_enc(t) = (t × 37) % 1440
...
/* frame_enc: time t → position on 1440 timeline */
static inline uint16_t frame_enc(uint32_t t) {
    return (uint16_t)((t * FRAME_STRIDE) % FRAME_CYCLE);
}

/* frame_at: enc → DualFrame (angle, mirror) */
typedef struct { uint16_t enc; uint8_t angle; uint8_t mirror; } DualFrame;
static inline DualFrame frame_at(uint16_t enc) {
    DualFrame f;
    f.enc = enc;
    f.angle = (uint8_t)(enc % 12);
    f.mirror = (uint8_t)((enc / 12) % 2);
    return f;
}

/* frame_seek: time t → DualFrame */
 ...
 * 2. node_id → clock_tick (1440 positions)
 * 3. clock_tick → frame_seek tile (stride-37)
 *
 * This gives: weight → unique tile on 1440 grid.
 */

/* Weight to tessellation node (same as beam_tess_codec.c) */
...
/* ══════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════
 *
 * Storage: 11 bits per weight (tile on 1440 grid)
 * 32 weights × 11 bits = 352 bits = 44 bytes
 * + 1 byte header = 45 bytes
 * vs Q8_0: 34 bytes (32 weights × 8 bits + 64 bytes scale)
 */

#define TILE_BITS 11  /* 1440 tiles needs 11 bits */
...
{
    printf("╔══════════════════════════════════════════════════════════╗\n");
    printf("║  Beam Tile Seek — Tessellation → Frame Seek            ║\n");
    printf("║  20736 nodes → 1440 tiles (stride-37)                 ║\n");
    printf("║  weight → tessellation → clock_tick → tile             ║\n");
    printf("╚══════════════════════════════════════════════════════════╝\n\n");
