#!/usr/bin/env python3
"""
GeoPixel One-Stop Service v2
═══════════════════════════════

Supports dynamic cube sizes with base 4, 8, 16 scaling.
No size limit — files of any size can be processed.

Usage:
    python geopixel_service_v2.py
"""

import tkinter as tk
from tkinter import ttk, filedialog, messagebox
import os, sys, time, hashlib, threading, math

sys.path.insert(0, os.path.dirname(__file__))

from hamburger_codec_poc_v5 import (
    unfold_cube, reconstruct_from_timeline_vectorized, 
    FRAME_CYCLE, FRAME_STRIDE
)

# ── Scale bases (4, 8, 16) ──
SCALE_BASES = [4, 8, 16]

def calc_cube_size(file_size, base=8):
    """Calculate cube side length based on file size and scale base."""
    if file_size == 0:
        return base
    
    # Calculate minimum side to fit file
    min_side = math.ceil(file_size ** (1/3))
    
    # Round up to next power of base
    power = 0
    while base ** power < min_side:
        power += 1
    
    side = base ** power
    
    # Ensure minimum size
    if side < base:
        side = base
    
    return side

def calc_compression(file_size, side):
    """Calculate compression ratio."""
    cube_voxels = side ** 3
    face_voxels = 6 * side * side
    return cube_voxels / face_voxels if face_voxels > 0 else 0

class GeoPixelService:
    """One-stop service for GeoPixel encoding/decoding."""
    
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("GeoPixel Service v2")
        self.root.geometry("700x600")
        self.root.configure(bg='#1a1a2e')
        
        self.input_path = tk.StringVar()
        self.output_path = tk.StringVar()
        self.scale_base = tk.IntVar(value=8)
        self.status = tk.StringVar(value="Ready")
        self.info = tk.StringVar(value="")
        
        self.setup_ui()
    
    def setup_ui(self):
        """Setup the UI."""
        style = ttk.Style()
        style.theme_use('clam')
        style.configure('TFrame', background='#1a1a2e')
        style.configure('TLabel', background='#1a1a2e', foreground='white')
        style.configure('TButton', background='#16213e', foreground='white')
        style.configure('TEntry', background='#0f3460', foreground='white')
        style.configure('TRadiobutton', background='#1a1a2e', foreground='white')
        
        main_frame = ttk.Frame(self.root, padding="20")
        main_frame.pack(fill=tk.BOTH, expand=True)
        
        # Title
        title = ttk.Label(main_frame, text="GeoPixel Service v2", 
                         font=('Helvetica', 16, 'bold'))
        title.pack(pady=(0, 20))
        
        # Input
        input_frame = ttk.Frame(main_frame)
        input_frame.pack(fill=tk.X, pady=5)
        ttk.Label(input_frame, text="Input:").pack(side=tk.LEFT)
        ttk.Entry(input_frame, textvariable=self.input_path, width=50).pack(side=tk.LEFT, padx=5)
        ttk.Button(input_frame, text="Browse", command=self.browse_input).pack(side=tk.LEFT)
        
        # Output
        output_frame = ttk.Frame(main_frame)
        output_frame.pack(fill=tk.X, pady=5)
        ttk.Label(output_frame, text="Output:").pack(side=tk.LEFT)
        ttk.Entry(output_frame, textvariable=self.output_path, width=50).pack(side=tk.LEFT, padx=5)
        ttk.Button(output_frame, text="Browse", command=self.browse_output).pack(side=tk.LEFT)
        
        # Scale base selection
        scale_frame = ttk.Frame(main_frame)
        scale_frame.pack(fill=tk.X, pady=10)
        ttk.Label(scale_frame, text="Scale Base:").pack(side=tk.LEFT)
        for base in SCALE_BASES:
            ttk.Radiobutton(scale_frame, text=str(base), variable=self.scale_base, 
                           value=base, command=self.update_info).pack(side=tk.LEFT, padx=10)
        
        # Mode selection
        mode_frame = ttk.Frame(main_frame)
        mode_frame.pack(fill=tk.X, pady=5)
        self.encode_var = tk.BooleanVar(value=True)
        ttk.Radiobutton(mode_frame, text="Encode", variable=self.encode_var, 
                       value=True).pack(side=tk.LEFT, padx=10)
        ttk.Radiobutton(mode_frame, text="Decode", variable=self.encode_var, 
                       value=False).pack(side=tk.LEFT, padx=10)
        
        # Info label
        ttk.Label(main_frame, textvariable=self.info, 
                 font=('Consolas', 9), foreground='#3498db').pack(pady=5)
        
        # Process button
        self.process_btn = ttk.Button(main_frame, text="Process", command=self.process)
        self.process_btn.pack(pady=10)
        
        # Status
        ttk.Label(main_frame, textvariable=self.status, 
                 font=('Helvetica', 10)).pack(pady=5)
        
        # Log
        self.log = tk.Text(main_frame, height=12, bg='#0f3460', fg='white',
                          font=('Consolas', 9))
        self.log.pack(fill=tk.BOTH, expand=True, pady=10)
    
    def update_info(self):
        """Update info based on input file and scale base."""
        input_path = self.input_path.get()
        if not input_path or not os.path.exists(input_path):
            self.info.set("")
            return
        
        file_size = os.path.getsize(input_path)
        base = self.scale_base.get()
        side = calc_cube_size(file_size, base)
        ratio = calc_compression(file_size, side)
        
        self.info.set(
            f"File: {file_size:,} bytes | "
            f"Base: {base} | "
            f"Cube: {side}³ = {side**3:,} voxels | "
            f"Ratio: {ratio:.1f}x"
        )
    
    def browse_input(self):
        path = filedialog.askopenfilename(title="Select Input File")
        if path:
            self.input_path.set(path)
            base, ext = os.path.splitext(path)
            self.output_path.set(base + ".geopixel")
            self.update_info()
    
    def browse_output(self):
        path = filedialog.asksaveasfilename(
            title="Select Output File",
            defaultextension=".geopixel"
        )
        if path:
            self.output_path.set(path)
    
    def log_message(self, msg):
        self.log.insert(tk.END, msg + "\n")
        self.log.see(tk.END)
        self.root.update_idletasks()
    
    def process(self):
        input_path = self.input_path.get()
        output_path = self.output_path.get()
        
        if not input_path or not os.path.exists(input_path):
            messagebox.showerror("Error", "Please select a valid input file")
            return
        
        if not output_path:
            messagebox.showerror("Error", "Please select an output file")
            return
        
        threading.Thread(target=self._process_thread, 
                        args=(input_path, output_path), 
                        daemon=True).start()
    
    def _process_thread(self, input_path, output_path):
        try:
            self.status.set("Processing...")
            self.process_btn.config(state='disabled')
            
            base = self.scale_base.get()
            
            self.log_message(f"Reading: {input_path}")
            t0 = time.perf_counter()
            data = open(input_path, 'rb').read()
            t_read = time.perf_counter() - t0
            self.log_message(f"  Size: {len(data):,} bytes ({len(data)/1024/1024:.2f} MB)")
            self.log_message(f"  Read: {t_read*1000:.1f} ms")
            
            if self.encode_var.get():
                self.log_message("\n--- ENCODE ---")
                self._encode(data, output_path, base)
            else:
                self.log_message("\n--- DECODE ---")
                self._decode(data, output_path)
            
            self.status.set("Done!")
            self.log_message("\n✓ Complete!")
            
        except Exception as e:
            self.status.set(f"Error: {e}")
            self.log_message(f"\n✗ Error: {e}")
            import traceback
            self.log_message(traceback.format_exc())
        finally:
            self.process_btn.config(state='normal')
    
    def _encode(self, data, output_path, base=8):
        import numpy as np
        
        n_bytes = len(data)
        side = calc_cube_size(n_bytes, base)
        total_voxels = side ** 3
        
        self.log_message(f"Base: {base}")
        self.log_message(f"Cube: {side}×{side}×{side} = {total_voxels:,} voxels")
        
        # Convert to cube
        self.log_message("Converting to cube...")
        t0 = time.perf_counter()
        
        padded = data + b'\x00' * (total_voxels - n_bytes)
        arr = np.frombuffer(padded, dtype=np.uint8)
        cube = arr.reshape(side, side, side).astype(np.float32)
        
        t_convert = time.perf_counter() - t0
        self.log_message(f"  Time: {t_convert*1000:.1f} ms")
        
        # Unfold to 6 faces
        self.log_message("Unfolding to 6 faces...")
        t0 = time.perf_counter()
        faces = unfold_cube(cube)
        t_unfold = time.perf_counter() - t0
        
        face_bytes = sum(f.nbytes for f in faces.values())
        self.log_message(f"  Faces: 6 × {side}×{side} = {face_bytes:,} bytes")
        self.log_message(f"  Time: {t_unfold*1000:.1f} ms")
        
        # Save
        self.log_message("Saving...")
        t0 = time.perf_counter()
        
        with open(output_path, 'wb') as f:
            f.write(b'GPIX')  # Magic
            f.write(np.uint32(base).tobytes())  # Scale base
            f.write(np.uint32(side).tobytes())  # Side length
            f.write(np.uint32(n_bytes).tobytes())  # Original size
            
            for face_name in ['front', 'back', 'left', 'right', 'top', 'bottom']:
                f.write(faces[face_name].astype(np.float32).tobytes())
        
        t_save = time.perf_counter() - t0
        output_size = os.path.getsize(output_path)
        self.log_message(f"  Saved: {output_path}")
        self.log_message(f"  Output size: {output_size:,} bytes")
        self.log_message(f"  Time: {t_save*1000:.1f} ms")
        
        # Summary
        total_time = t_convert + t_unfold + t_save
        ratio = cube.nbytes / face_bytes
        self.log_message(f"\n--- Summary ---")
        self.log_message(f"Original: {n_bytes:,} bytes")
        self.log_message(f"Stored:   {face_bytes:,} bytes (6 faces)")
        self.log_message(f"Ratio:    {ratio:.1f}x")
        self.log_message(f"Total:    {total_time*1000:.1f} ms")
    
    def _decode(self, data, output_path):
        import numpy as np
        
        self.log_message("Reading header...")
        magic = data[:4]
        if magic != b'GPIX':
            self.log_message("✗ Invalid format")
            return
        
        base = int(np.frombuffer(data[4:8], dtype=np.uint32)[0])
        side = int(np.frombuffer(data[8:12], dtype=np.uint32)[0])
        orig_size = int(np.frombuffer(data[12:16], dtype=np.uint32)[0])
        
        self.log_message(f"  Base: {base}")
        self.log_message(f"  Side: {side}")
        self.log_message(f"  Original: {orig_size:,} bytes")
        
        # Read 6 faces
        self.log_message("Reading 6 faces...")
        t0 = time.perf_counter()
        
        faces = {}
        offset = 16
        for face_name in ['front', 'back', 'left', 'right', 'top', 'bottom']:
            face_data = data[offset:offset + side*side*4]
            faces[face_name] = np.frombuffer(face_data, dtype=np.float32).reshape(side, side)
            offset += side * side * 4
        
        t_read = time.perf_counter() - t0
        self.log_message(f"  Time: {t_read*1000:.1f} ms")
        
        # Reconstruct
        self.log_message("Reconstructing...")
        t0 = time.perf_counter()
        reconstructed = reconstruct_from_timeline_vectorized(faces, side)
        t_recon = time.perf_counter() - t0
        self.log_message(f"  Time: {t_recon*1000:.1f} ms")
        
        # Convert back
        arr = reconstructed.astype(np.uint8).flatten()
        output_data = arr[:orig_size].tobytes()
        
        with open(output_path, 'wb') as f:
            f.write(output_data)
        
        self.log_message(f"  Saved: {output_path}")
        self.log_message(f"  Size: {len(output_data):,} bytes")
        
        total_time = t_read + t_recon
        self.log_message(f"\n--- Summary ---")
        self.log_message(f"Input:    {len(data):,} bytes")
        self.log_message(f"Output:   {len(output_data):,} bytes")
        self.log_message(f"Total:    {total_time*1000:.1f} ms")
    
    def run(self):
        self.root.mainloop()

if __name__ == '__main__':
    app = GeoPixelService()
    app.run()
