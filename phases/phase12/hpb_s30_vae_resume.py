...
RESUME_PATH = "hpb_s30_checkpoint.pt"
...
def train(image_paths: list, epochs: int = 50, domain: str = "default", resume: bool = False):
    print(f"\n── S30 Train ─────────────────────────────────────")
    print(f"  Images: {len(image_paths)}  Epochs: {epochs}  Domain: {domain}  Device: {DEVICE}")
    ...
    print(f"  Model: {model.model_size_kb():.1f}KB  Tiles/epoch: {len(dataset)}  Batch: {batch_size}")
    print()
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        if (epoch - start_epoch + 1) % 10 == 0 or epoch == start_epoch:
            print(f"  Epoch {epoch+1:4d}  loss={avg_loss:.4f}  L1={avg_l1:.4f}  "
                  f"PSNR~{psnr:.1f}dB  lr={scheduler.get_last_lr()[0]:.5f}")

        if avg_loss < best_loss:
            best_loss = avg_loss
            _save_model(model, domain, avg_l1, epoch, optimizer, scheduler, best_loss)

    print(f"\n  Best loss: {best_loss:.4f}  →  {MODEL_PATH}")
    _print_model_stats(model)

def _save_model(model: TinyVAE, domain: str, val_l1: float, epoch: int = 0,
                optimizer=None, scheduler=None, best_loss: float = 0.0):
    ckpt = {
    ...
    if optimizer:  ckpt["optimizer_state"] = optimizer.state_dict()
    if scheduler:  ckpt["scheduler_state"] = scheduler.state_dict()
    torch.save(ckpt, MODEL_PATH)
    # also save full resume checkpoint
