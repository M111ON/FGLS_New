...
def bench(src_path: str):
    ...
    # ── JPEG + threshold residual ──
    print(f"\n── JPEG + threshold residual ──")
    for q in [80, 85]:
        buf = io.BytesIO()
        Image.fromarray(rgb).save(buf, format='JPEG', quality=q, optimize=True)
        jbytes = buf.getvalue()
        jpeg_u8 = np.array(Image.open(io.BytesIO(jbytes)).convert('RGB'))
        delta = rgb.astype(np.int16) - jpeg_u8.astype(np.int16)
        for thr in [20, 24,28]:
            masked = delta.copy()
            masked[np.abs(masked) < thr] = 0
            res_z = _compress(masked.astype(np.int8).tobytes())
            final = np.clip(jpeg_u8.astype(np.int16) + masked, 0, 255).astype(np.uint8)
            p = psnr_np(rgb, final)
            total_kb = (len(jbytes) + len(res_z)) // 1024
            zero_pct = int(100 * np.sum(masked==0) / masked.size)
            print(f"  q={q} thr={thr:2d} → {total_kb}KB  PSNR={p:.1f}dB  zeros={zero_pct}%")
