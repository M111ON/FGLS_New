...
/*
 * Decision table:
 *   sign XOR  == 0  → same polarity
 *   angle diff == 0 → aligned
 *   letter match    → LetterPair coupled
 *
 *   same_polarity AND aligned AND coupled  → WARP  (fast path)
 *   same_polarity AND aligned              → ROUTE (normal)
 *   same_polarity AND NOT aligned          → COLLISION (drift)
 *   NOT same_polarity                      → GROUND (bypass geo)
 */
...
/*
 * lc_gate_assign: assign LetterPair to face from (addr, fibo_seed)
 *   letter_idx = addr % 26
 *   slope_hash = fibo_seed ^ (addr >> 8)
 *   core       = derive from addr + face_id
 */
...
/*
 * lc_gate_match: check if two faces are coupled
 *   angle_ok  = (core_a ^ core_b ^ slope_hash) low 16 bits == 0
 *   letter_ok = same LetterPair index
 */
...
/*
 * lc_gate_force_couple: scan all face pairs, couple on match
 * Returns number of new couplings (0..6)
 */
static inline uint8_t lc_gate_force_couple(LCTwinGateCtx *g) {
    uint8_t coupled = 0;
    for (uint8_t fa = 0; fa < LC_FACES && g->coupled_count < LC_FACES; fa++) {
        if (g->faces[fa].coupled) continue;
        for (uint8_t fb = (uint8_t)(fa + 1); fb < LC_FACES; fb++) {
            if (g->faces[fb].coupled) continue;
            if (lc_gate_match(g, fa, fb)) {
                g->faces[fa].coupled = 1;
                g->faces[fb].coupled = 1;
                g->coupled_count    += 2;
                coupled             += 2;
            }
        }
...
/*
 * lc_gate_update: called per write — assign face from trit, try couple
 *   trit  = (addr ^ value) % 27 → face = trit % 6
 */
static inline uint8_t lc_gate_update(LCTwinGateCtx *g,
                                       uint64_t addr,
                                       uint64_t value,
                                       uint64_t fibo_seed)
{
    uint8_t face = (uint8_t)((addr ^ value) % LC_FACES);
    lc_gate_assign(g, addr, fibo_seed, face);
    return lc_gate_force_couple(g);
}
