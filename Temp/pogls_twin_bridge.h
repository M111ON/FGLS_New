...
/* ── constants ──────────────────────────────────────────────────── */
#define TWIN_BRIDGE_VERSION   11u
    ...
    PipelineResult *res_out)
{
    ...
    /* ── LC polarity gate ───────────────────────────────────────────
     * Encode addr/value → LCHdr, judge gate decision.
     * GROUND: bypass geo entirely → trit-mapped dodeca_insert.
     * COLLISION: bump drift_acc, fall through to geo.
     * WARP/ROUTE: normal geo path (WARP = fast intersect hint).    */
    LCHdr hA = lc_hdr_encode_addr(addr);
    LCHdr hB = lc_hdr_encode_value(value);
    LCGate gate = lch_gate(hA, hB,
                           b->lc_gate.palette_a,
                           b->lc_gate.palette_b);

    if (gate == LC_GATE_GROUND) {
        /* P0 fix: trit decomposition — not raw mask */
        ...
        DodecaEntry *de = dodeca_insert(&b->dodeca,
                      (uint64_t)trit,
                      0, 0, off, spoke, 0);
        /* O2: GROUND → fts_write direct */
        fts_write(&b->store, addr, value, de);
        b->lc_ground++;
        b->twin_writes++;
        b->total_ops++;
        /* O1: update LetterPair coupling */
        lc_gate_update(&b->lc_gate, addr, value, b->fibo.seed.gen2);
        return ev;
    }

    if (gate == LC_GATE_COLLISION) {
        b->flow.drift_acc += 8u;
        b->lc_collision++;
    } else if (gate == LC_GATE_WARP) {
        b->lc_warp++;
    } else {
        b->lc_route++;
    }
    ...
    if (at_end) {
        ...
        DodecaEntry *de = dodeca_insert(&b->dodeca, r_next, 0, 0, offset,
                      b->flow.hop_count, 0);
        /* O2: route/warp path → fts_write */
        fts_write(&b->store, addr, value, de);
        /* O1: update LetterPair coupling */
        lc_gate_update(&b->lc_gate, addr, value, b->fibo.seed.gen2);
        b->twin_writes++;
...
static inline void twin_bridge_flush(TwinBridge *b)
{
    ...
    DodecaEntry *de = dodeca_insert(&b->dodeca, b->flow.route_addr, 0, 0,
                                     offset, b->flow.hop_count, 0);
    /* flush → fts_write with sentinel addr=route_addr */
    fts_write(&b->store, b->flow.route_addr, 0, de);
    fts_serialize(&b->store);
    b->twin_writes++;
    b->flush_count++;
    diamond_flow_init(&b->flow);
}
...
typedef struct {
    ...
    uint32_t lc_ground;
    uint32_t lc_collision;
    uint32_t lc_coupled;       /* faces coupled via LetterPair */
    uint32_t fts_active_cosets;
    uint32_t fts_overflows;
    float    write_density;
} TwinBridgeStats;
...
static inline TwinBridgeStats twin_bridge_stats(const TwinBridge *b)
{
    ...
    s.lc_warp          = b->lc_warp;
    s.lc_route         = b->lc_route;
    s.lc_ground        = b->lc_ground;
    s.lc_collision     = b->lc_collision;
    s.lc_coupled       = b->lc_gate.coupled_count;
    FtsTwinStats fs    = fts_stats(&b->store);
    s.fts_active_cosets= fs.active_cosets;
    s.fts_overflows    = fs.overflows;
