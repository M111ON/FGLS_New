/* ═══════════════════════════════════════════════════════════════════════════
 * kis_geom_codec_test.c — Geometric codec test
 * ═══════════════════════════════════════════════════════════════════════════ */
...
static void test_all_same(void) {
    ...
    memset(w, 42, n);
    uint32_t mismatches = kis_geom_roundtrip_test(w, n);
    uint32_t buf_size = n + 8192;
    uint8_t *buf = (uint8_t *)malloc(buf_size);
    uint32_t enc = kis_geom_encode(w, n, buf, buf_size);
...
static void test_sequential(void) {
    uint32_t n = 256;
    int8_t *w = (int8_t *)malloc(n);
    for (uint32_t i = 0; i < n; i++) w[i] = (int8_t)i;
    uint32_t mismatches = kis_geom_roundtrip_test(w, n);
    uint32_t buf_size = n + 8192;
    uint8_t *buf = (uint8_t *)malloc(buf_size);
    uint32_t enc = kis_geom_encode(w, n, buf, buf_size);
    T(3, "Sequential 0..255 — roundtrip", mismatches == 0);
    printf("    codec: %u bytes, raw: %u bytes, ratio: %.1fx\n", enc, n, (double)n / enc);
    free(w); free(buf);
}
...
static void test_single(void) {
    ...
    uint8_t buf[8192];
    int8_t decoded = 0;
    uint32_t enc = kis_geom_encode(&w, 1, buf, sizeof(buf));
    int rc = kis_geom_decode(buf, enc, &decoded, 1);
...
static void test_real_gguf(const char *path, uint32_t max_weights) {
    ...
    for (uint64_t ti = 0; ti < gf.tensor_count; ti++) {
        ...
        uint32_t buf_size = n + 8192;
        uint8_t *buf = (uint8_t *)malloc(buf_size);
        uint32_t enc = kis_geom_encode(weights, n, buf, buf_size);
        clock_t t1 = clock();

        int8_t *decoded = (int8_t *)malloc(n);
        int rc = kis_geom_decode(buf, enc, decoded, n);
        clock_t t2 = clock();
...
int main(int argc, char **argv) {
    printf("╔══ KIS GEOM CODEC — Geometric Formula Test ══╗\n\n");
    ...
    if (argc > 1) {
        test_real_gguf(argv[1], 0);
    } else {
        test_real_gguf("I:/model/Kokoro_no_espeak_Q8.gguf", 100000);
    }
