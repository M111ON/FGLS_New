#!/bin/bash
# ──────────────────────────────────────────────────────────────────────
# colab_bandwidth_bench.sh — Deploy + Run GPU Jet Puller Bandwidth Bench
# ──────────────────────────────────────────────────────────────────────
# Usage:
...
BENCH_SRC="${BENCH_DIR}/gpu_jet_puller_bench.cu"
DEPLOY_DIR="/tmp/colab_bandwidth"

echo "═══════════════════════════════════════════════════════"
echo "  GPU Jet Puller — Bandwidth Bench Deploy (Colab T4)"
echo "═══════════════════════════════════════════════════════"
...
result = subprocess.run([
    ...
    "-o", "gpu_jet_puller_bench",
    "gpu_jet_puller_bench.cu",
    "-lm"
...
result = subprocess.run(["./gpu_jet_puller_bench"], capture_output=True, text=True, timeout=600)
...
for line in result.stdout.split('\n'):
    if 'Throughput' in line or 'CONFIG' in line or 'optimal' in line.lower() or 'Results' in line:
        print(f"  {line.strip()}")
PYEOF
...
cd "${DEPLOY_DIR}"
tar czf /tmp/gpu-jet-bandwidth.tar.gz \
  gpu_jet_puller_bench.cu \
  collection/ \
  run_bench.py
echo "  └─ Created: /tmp/gpu-jet-bandwidth.tar.gz (${SIZE:-?} bytes)"
...
echo "═══════════════════════════════════════════════════════"
echo ""
echo "  hermes colab new -s bandwidth --gpu T4"
echo "  hermes colab upload -s bandwidth /tmp/gpu-jet-bandwidth.tar.gz /content/"
echo ""
echo "  Then in Colab:"
echo "    cd /content && tar xzf gpu-jet-bandwidth.tar.gz"
